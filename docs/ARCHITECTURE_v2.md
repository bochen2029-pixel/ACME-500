...
| **D** | HOLD and STRATUM rows on change, not per period; the TICK-driven loop; the durable tape with checkpoint and `--dump` on the observer's contract; switch and pill; the gate as a hashed library; O18, O19, the dynamic O25, O30 (every EFFECT re-derives from rows) | a kill survives | identical under `live`; §14b |
