...
| the standing spec | `docs/ARCHITECTURE_v4.md` (mirror `C:\55555\ACME-500_INTELLECT-500_ARCHITECTURE_v4_2026-09-15_FABLE5-1.md`; update both), version 4.1: a superset whose Part II is `ARCHITECTURE_v3.md` verbatim (itself a superset of v2 verbatim: §14 the build order, §14a the amendments, §14b step D and E0, §15 kills 1–16; v3's §18–§23 the alphabet thesis, the archive plane, the `TapeJudge`, O32–O36, kills 17–19). v4's Part I §24–§41 adds the fifth world (the closed firm behind the port: the selection rule, the bootstrap rule, the floor), lanes (the tape of tapes), the constitution in code (the licensor as a process, the stake, the production battery), the ladder v2 with the calibration harness, counterparty planes and installments, tenancy, lineage, models behind lanes and the replay with no model, compile-out, the account, the standard, O31–O58, kills 20–27, and §39.2 the order as confirmed by the operator on 2026-09-15. Standing since that confirmation; v3 and v2 stay on disk unedited. |
| the receipts | `receipts/RECEIPT_ACME500_STEP-*.md` (one per step, dated) with raw outputs under `receipts/step-*-<date>/<sub-step>/`; the last landed is `RECEIPT_ACME500_STEP-E_2026-09-15.md` (E0) |
| the rules for this repo | `CLAUDE.md` = `AGENTS.md` (mirrored; keep both identical) |
| the brainstorms (operator's folder) | `C:\55555\NOTABLE-FROM-THE-FINAL-BRAINSTORM_2026-09-14_OPUS5.md`, `BRAINSTORM_ACME-AS-A-RUNTIME_REVIEW_2026-09-14_OPUS5.md`, `BRAINSTORM_DECISION-PRESERVING-COMPRESSION_REVIEW_2026-09-15_OPUS5.md`, `BRAINSTORM_CONCESSIONS-AND-THE-WAY-FORWARD_2026-09-15_OPUS5.md`, `BRAINSTORM_THE-MACHINE-NATIVE-IR-OF-AN-ORGANIZATION_2026-09-15_OPUS5.md`, `BRAINSTORM_ACME-v4_THE-CLOSED-FIRM-AS-A-WORLD_2026-09-15_FABLE5-1.md` (the brainstorm behind v4; the ten `C:\ONE` documents and `C:\ONE\NINETY-PERCENT_2026-09-15.pdf` are its sources) |
...
**Landed on `v2`, all pushed:** A, B (+remediation), C0 (+prints, +QC), C1, D0, D1, D2+D3; the docs-only concessions commit; ARCHITECTURE v3; the handoff and `tools/run_step.sh`; **E0** (the shared fact and the judge's certificate, unresolved outcomes with the write-off rule, outstanding exposure with the writ's cap; O37 lie 24, O38 lie 25); the QC commit after E0 with ARCHITECTURE v4.1 staged as standing; **E3a** (`--o31`: the boundary judge in the plant and the false-promotion rate of the ladder as built, the "before": 13.7 percent of bands under a judge a delta worse than the incumbent at 500 seats, against a stated 5; `tools/run_step.sh` runs the lie arms six at a time). See the last section of `receipts/RECEIPT_ACME500_STEP-E_2026-09-15.md` for the head.
...
**Open findings** (recorded, not fixed): F11 novelty inert (the score is a per-class constant; fix with O22 in E1, with the conformal construction `(k+1)/(n+1)`); F12 the frontier re-rented daily for the same escalated cell (the counsel memo, O23, E1); F14 the uniform floor cannot bind (the retained stratum, O20, now E3); F21 the ladder's floor paid once, not per rung (6 of 10 rung-5 bands under n₀; the ladder v2 of E3 replaces the rule); **F23 the e-process's null is a plug-in baseline that resets on promotion — no anytime guarantee is established for the rule as built; O31 (E3) measures its false-promotion rate before and after the repair, and no lane goes `live` in I until O31 has passed.** F22 (a coin judge admitted from history) was found and fixed in C1 (Cohen κ ≥ 0.2).
...
1. Reproduce the battery before editing: 28/28, every lie arm seen to catch its lie, O17/O28/O25a/O30a green. A nonzero exit for an unrelated reason is not a caught lie. Never report green without the lie.
2. Every step ends in a dated receipt with commands, outputs, exit codes, denominators, and *what was not done*. Readings are identical between steps unless the step is one allowed to move numbers (C1 and E0 were; E3 and F1 will be; F0 must not), and then every delta is named with its cause.
...
v4.1 is standing and E0 is landed. The order from here was confirmed by the operator on 2026-09-15 (v4 §39.2); each sub-step lands with its receipt, commit and push:
...
| **E3** (v4 §28, §25.4) | opened by an O31 reading against the ladder as built (the false-promotion rate under a boundary judge: the "before"); then the ladder v2 (the bar frozen per term at its upper bound from m_min paired outcomes; one evidence process per class-band-term, never reset, rungs at α·2^−k; mirrored demotion on all machine outcomes; term and lapse; REGIME; n₀ printed, not a condition), the retained stratum at every rung routed to the incumbent (O20, F14), the calibration harness (`calib.h`: the curve per licence key; unmeasured reads rung 0; O47); O31 (after), O31b (under the shared fact), O31c (with an installment channel), O45 (the ladder as a fold); F-STUB and F-CALIB run | the false-promotion rate at its level over seeds, before and after, named; numbers move (F21's bands re-earn) and every delta is named |
| **F0** (v4 §26, §27, §32.5, §34) | lanes and the SEAL row; `acme-licensor` as a process with the salt, the stake, STOP, REGIME; fail closed on a missing STRATUM row; RESERVE and `--executor-loss`; the instrument lane and the production battery; `--replay-from` with every judge lane absent; the two-process sim; O42, O43, O44, O53, O55, O58 | **every reading identical to E3 under `live`**, in-process and across processes; a licensor kill survived; O1b across processes |
| **F1a** (v4 §25) | `--select`, `WorldParams` from selection parameters, the greenfield world with the bootstrap rule and the floor as the null arm, `--absent N`; O39, O40, O41; F-SELECT and F-BOOT run; business A against ACME's own world | ACME's own world reproduces E3 byte for byte under `WorldParams`; business A's head licenses within predicted D_c; business C prints c_c ≤ 0 |
| **F1b** (v4 §29, §35) | `--pair` and INSTALLMENT with the grading channel; `--wire-order`; EXPECT rows, `--anomalies`, `--backtest`, `--account`; O48, O49, O56 | a paired class's latency collapses to the tick; the wire order's curve against `--select`'s projection |
| **E1′** | the counsel memo (O23, F12), VOC as the frontier's ranking, novelty on the frame with the conformal fix (O22, F11), O-TEMPLATE, O-GEOM′, the coupling print, the latency-and-value sweep | numbers move and are named |
| **E2** (v3 §19) | the archive plane behind a lane, with the planted embedder for the battery; `SPAN`, `WANT`, `RF_DISCOVERED`; O32–O35; F-EMBED | the controller blind to the plant |
| **G0, G** | the schema as data; the census on one real lane at `off`, no judge, no effect, the plane visible (the first product); may start any time, in parallel | the compiler meets a world |
| **H, H2, I** | as v4 §39: the gym with `--export` and `--do`; tenancy, federation, lineage, compile-out, the anomaly schedule; the first real lane with the conformance suite | the first exterior verdict; **no lane `live` until O31 has passed** |

Each sub-step: build → battery → all lie arms → tool oracles → three modes diffed against the previous stage → a section in the step's receipt → docs → commit → push. No further version of the architecture until something in v4 has a receipt.

## 8 · The operator, and how to work with them

- One step at a time; read only what is named; do not scan ahead. When a message says brainstorm, write the file to `C:\55555` first (`<TITLE>_<YYYY-MM-DD>_<MODEL>.md`: `_OPUS5`, `_FABLE5-1`) and present it in full in chat; when it says proceed, proceed and report with numbers; when it says review first, stop at the document.
...
1. Read this file. 2. `git -C C:/55555/acme500 log --oneline -12` and `git status -sb` — the branch must be `v2`, clean, in sync. 3. Read the latest receipt's last section and the spec's §14a/§14b tail. 4. `tools/build_wsl.sh`, then `./acme --selftest` — expect 28 of 28 in ~90 s. 5. If the task is E3, F0 or F1, read v4 §24–§41 (§39.2 is the confirmed order) and the section named for the step; if it is E1′ or E2, read v3 §18–§23 as well; if it is docs, read `CLAUDE.md`. 6. Say what you are about to do and wait for the word.
