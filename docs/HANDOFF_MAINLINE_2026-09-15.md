...
| the standing spec | `docs/ARCHITECTURE_v2.md` (mirror `C:\55555\ACME-500_INTELLECT-500_ARCHITECTURE_v2_2026-09-14_OPUS5.md`; update both) — §14 the build order, §14a the amendments (items 6–8: C0, C1 landed; the five claims re-graded), §14b step D's plan and E0, §15 the kills 1–16 |
| the spec under review | `docs/ARCHITECTURE_v3.md` (mirror `..._v3_2026-09-15_OPUS5.md`): a superset — Part II is v2 verbatim; Part I §18–§23 adds the alphabet thesis, the archive plane (embedders/rerankers behind the store), the `TapeJudge`, the rows `WANT`/`SPAN`/`RF_DISCOVERED`, O32–O36, kills 17–19, and E2 in the build order. **The operator has not yet approved v3; nothing in it is built until they do.** |
...
Pending the operator's review of v3. When they give the word, the order is:

| step | content | acceptance |
|---|---|---|
| **E0** | the shared-fact experiment: a shared determinant per class that moves mid-run; the judge returns a sensitivity per shared fact through the port; the kernel re-derives which cells' margins *and bands* the change crosses from the returned sensitivity and the memo (never the plant's weights) and reads only those; an outcome kind *unresolved* with a rate and an explicit write-off rule; contention for an exposure allowance; `Writ::exposure_cap` with a hold reason and three ledgers (reserved / committed / outstanding). The receipt counts separately: source interpretations · affected cases · incremental updates · full reads · changed dispositions and changed authorizations · discrepancies against the full reference · outstanding exposure over time. | the thesis in one print: reconsideration only where a margin is crossed; numbers move and each is named |
