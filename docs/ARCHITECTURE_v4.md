# ACME-500 / INTELLECT-500 · ARCHITECTURE v4

### One machine, five worlds, one tape, many lanes. The closed firm is a world behind the port; every writer is a lane; the licensor is a process the machine cannot reach; the ladder is repaired before anything new may act; a run replays with no model present; the alphabet, the tape and the battery are a standard a second implementation can be graded against; and the ladder of closure tests runs to the top rung with its status column honest.

**Version 4.1 · 2026-09-15 · Claude Fable 5.1 · the `C:\55555` session.** Revised the same day after reading NINETY PERCENT, the enterprise treatise, in full; the revision is listed in §24.5 and moves no reading. A **superset** of `ARCHITECTURE_v3.md`: Part II of this file is v3 as landed, verbatim and unedited, which carries its own Part I (§18–§23) and its own Part II (v2 verbatim, §0–§17 with §14a, §14b and the kills 1–16). Part I of this file is what v4 adds, numbered §24 onward so that every cross-reference into v3 and v2 holds. Nothing below the divider is overwritten; where v4 amends a statement it says so and leaves the original standing. The steps landed on branch `v2` at the time of writing: A, B, C0, C1, D0–D3, E0 (`receipts/RECEIPT_ACME500_STEP-E_2026-09-15.md`), 28 of 28 in the battery with lie arms 0–25, four tool oracles, every default reading identical to D2, rebuilt and re-run this morning at head `ea9ea74` (28 passed, 0 failed, 95.4 s).

**What v4 is for.** Ten documents in `C:\ONE` (2026-09-15, 09:29–10:36), the paper they produced, THE CLOSED FIRM (Revision 1), and the enterprise treatise that followed it, NINETY PERCENT, moved the programme's object from a treated firm to a born one, and then named the four dials that take an inherited one to a tenth of its headcount: an organization chosen so that its world grades it in hours, held by one writ, one salt, one switch and one stake, born in a simulator that is the product's own binary, with a licensor the machine cannot reach, able to reproduce by disposition alone. Their sharpest sentence is a fact about ACME: the act loop bought nothing measurable in a year because ACME's world answers in months, and a greenfield firm chooses its world. ACME is the only instrument on this disk that can test that lever at planted grade, and today it cannot, because the closed firm is not one of its worlds. v4 fits the ONE corpus into the architecture as built, as rows, interfaces, oracles and steps, so that: (1) the selection rule becomes a measurement ACME makes this month; (2) the constitution, tenancy, federation, counterparty planes and lineage rest on one mechanism, lanes; (3) the ladder is repaired in code, with its anytime guarantee stated for the procedure actually run, before any lane goes live; (4) a model of any kind attaches behind a lane and the run replays without it; (5) the alphabet, the tape format, the port and the battery become a standard. It also corrects the ONE corpus in seven places where the paper's mechanism does not survive the gate as written or the verdict law as the paper itself states it (§24.3).

**Tags** as in v2 and v3: **[M]** measured with a file · **[P]** planted · **[BUDGET]** arithmetic on stated inputs · **[AS BUILT]** code on this disk · **[v3]** as v3 specified · **[v4]** new in this version · **[UNRUN]** an experiment this document names and has not performed · **[ONE]** a claim of the ONE corpus, at the label the corpus gave it.

**Reading order for the review:** §24 (where v4 stands and what it refuses), §25 (the fifth world), §26 (lanes), §28 (the ladder repaired), §32 (models behind lanes; the run replays), §39 (the build order), then §36 (the standard) and §37 (scale), then Part II for everything v3 settled.

---

## §24 · Where v4 stands, and what it refuses **[v4]**

### 24.1 The state this version is written against

| item | value |
|---|---|
| source of record | `C:\55555\acme500`, branch `v2`, head `ea9ea74` (2026-09-15 08:15), clean, in sync with `github.com/bochen2029-pixel/ACME-500` |
| the battery | 28 of 28; lie arms 0–25 each exiting 0 with the lied-to oracle's own line PASS; O17 (both lies), O28, O25a, O30a green **[M, the step-E receipt; reproduced today]** |
| the pins | alphabet `0x81ebc266`; gate `aa7e850b6bdb032f` **[M]** |
| the readings | as v3 §14a item 7 and §14b E0: twin good 42.70 vs 38.06, wrong 8.40 vs 11.94, cost $1,837.7M vs $1,254.3M, bias +34.42; 32 bands from history, 29 past rung 0, 26 of 48 unlicensable, kappa 0.110, licensed mass 63.5 percent, N 193/182; reads 1,729 a period with 1,462 carried; frontier 986 a day; shadow agreement 87.2 percent; E0: 2,062 frames moved by a shared fact, 789 carried by the certificate, 1,273 read again; uncapped exposure peaks at $96.1M; the licence follows the cap ($50M → 29.4 percent, $25M → 0) **[M]** |
| open findings | F11, F12, F14 (E1); F21 the floor paid once; F23 the e-process's plug-in null with resets; O31 unrun; no lane goes `live` until it passes **[AS BUILT]** |
| the tree | 6,148 lines; `include/acme/*.h` and `src/main.cpp`; no dependency **[M]** |

### 24.2 The ONE corpus in one paragraph, and the four measurements it names

The corpus holds that the machine exists at instrument grade and is honest, that the glue that closes the loop is a fold and not a model, that presence and a deleted interior are the mechanism and superhuman judgment was never needed, that the heart consumes a judge's calibration curve and never the judge, that learning is dense and licensing is sparse, that the plane is a lattice because contention is local, that closeness is three distances at their floors inside two bounds, that only disposition crosses a gap, that refinement cannot change hills and the anomaly source is free, and that the ceiling on autonomy is the world's verdict rate so the firm must be chosen. It names four decisive measurements, all unrun: the first outcome row on a real lane; the ladder's false-promotion rate under a judge at the bar; the twin race between a resident center and a tick-driven one; and spore against tape. It names two more that decide sections: commit grain against the full stream, and the event model against the lattice at equal parameters. v4 makes the second runnable at planted grade in E3 (O31, O31b), the fourth's synthetic analogue in H2 (O51, F-GERM), the selection rule in F1 (O39, F-SELECT), and counterparty planes in F1 (O49); it cannot make the first or the third runnable, and it says so in §40.

### 24.3 Seven corrections to the ONE corpus, carried below

These are the places where the paper's mechanism does not survive the gate as built (v2 §4.3), the ladder as built (v2 §4.5), or the verdict law as the paper states it (THE CLOSED FIRM §VI.1). Each is fixed in the section named.

| # | the corpus says | why it fails as written | the fix, and where |
|---|---|---|---|
| 1 | the resident is attached "at rung one" and a chosen firm's act loop licenses in weeks | the gate holds every cell at rung 0; the evidence process counts unattended acts only; the only path to rung 1 in the code is history admission; a greenfield has no history. After eighty live days the world licensed nothing history had not **[M]** | the bootstrap rule, which is NINETY PERCENT's "one licensing rule per class, signed once" given the admission kind the code lacks: a reversible class-band admitted to rung 1 (canary-only) by that rule, under a bootstrap exposure cap; §25.3, O40 |
| 2 | where there is no incumbent the bar is the jury's rate | the jury never disposes, so its choices are never executed and it has no world-graded rate; a bar must come from executed choices | the floor is the null arm: the retained stratum executes the class's authored safe terminal, its outcomes are the paired baseline and the frozen bar; the jury is a veto on irreversibles and a control probe; §25.4, O41 |
| 3 | installments "count under the law once calibrated" | an installment entered as an estimated likelihood ratio at its point estimate is not a supermartingale factor; the anytime guarantee of §28 would be lost the day one entered the evidence process that way | installments narrow freely and widen only through a class's declared grading channel, whose calibration to the final verdict is frozen per term at its conservative bounds, one factor per cell, with the final verdict feeding demotion and the next term's calibration; an installment that is the final verdict is an OUTCOME at commit; §29.3, O48, O31c |
| 4 | learning is dense on the world's rows and the model learns the firm's dynamics | the world's rows under the firm's action are arrivals, installments, receipts and outcomes; the rest of the next state is the fold, exact and free; the dense head learns the world's arrival and verdict-timing process | the dense head is a forecaster that improves the field; the sparse head is the judge; §32.3, kill F-DENSE |
| 5 | pooling under exchangeability by a permutation test at 0.10 | failing to reject on a small window is low power taken as permission | a power guard: m_pool outcomes per tenant per window before any pooling; §30.1, O50 |
| 6 | the licensor forfeits the stake on a breach the battery catches | nothing runs the battery on a live lane | the production battery on the shadow stratum, its code pinned, lies planted by an instrument lane under their own provenance; §27.3, O44 |
| 7 | escalation of judgment ends in an experiment, never in a person | an experiment is an unattended act on a counterparty's promise and needs authority | the experiment is the bootstrap allowance seen from the judge's side: a reversible act on the canary stratum under the bootstrap cap with reason `experiment`; a reason, not a sixth verb; §25.3 |

### 24.4 The fence: what v4 refuses

Every section below is inside these, and each is guarded by an oracle that already exists or lands with the section.

1. **The kernel never includes the plant, the governor, or any model.** `ledger.h`, `port.h`, `license.h`, `gate.h`, `tapefile.h`, `solver.h`, `machine.h`, `report.h` and the units v4 adds (`lanes.h`, `calib.h`, `routine.h`, `account.h`) compile under `-DACME_NO_PLANT -DACME_NO_GOVERNOR`; models exist only behind lanes (O17, O58). The fix for a broken guard is never to weaken it.
2. **Nothing learned disposes; no vector is a key.** The gate reads a calibrated scalar and hashes; the memo, the licence and the re-derivation are keyed on hashes (v3 §18.4, F-VECTOR-KEY).
3. **Authority widens only on exterior rows from the world port on executed choices on a stratum the machine cannot predict; it narrows on anything.** Forecasts, rollouts, agreement, self-reports and certificates never widen; an installment widens only through the frozen channel of §29.3, and a final OUTCOME row is the verdict everywhere else (§28, §29.3).
4. **Every table is a fold; a run replays with no model present** (O14, O30, O53).
5. **The gym never licenses; a patch reaches the wire only by a paired arrived backtest carrying its inverse** (v3 §7.2, §35).
6. **The clock is a row; the machine reads the day, never the wall** (O25, O25a), and every lane's rows fold in a published order (O42).
7. **The writ is authored, never learned; hazards are unreachable by construction**: an irreversible class never acts below rung 4, never under the bootstrap rule, and never without a warrant row (v2 §4.3, O40).
8. **The instrument can be lied to and is seen to catch the lie**, in the sim and on the live lane (O44).
9. **Fail closed.** A cell without a STRATUM row holds; a silent licensor holds everything; a STOP row halts acting before the period reads (O43).
10. **No constant is tuned to pass an oracle; every number moved is named with its cause in a receipt.**

### 24.5 Revision 4.1 · after NINETY PERCENT

NINETY PERCENT (the enterprise treatise, 2026-09-15, read in full the same day) names the verdict rate's four dials (tolerance, pooled volume, installments, counterparty planes), the order rule (automate wires in order of D_c so that licensed mass rises from the first quarter), the forward-deployed engineer's six deliverables and the enterprise's five phases, the shared object with no model as the first product, the weekly pair of numbers with its two counterweights, and the tower of shadows as a bill forecast. Everything in it that v4.0 already held stands as it stood. What it adds that v4.0 lacked is carried in seven places, none of which changes a reading:

1. §29.3 restated: installments widen through a frozen conservative calibration, one factor per cell. v4.0 forbade widening, which was safe and too weak; dial three is restored with its guarantee intact. O48 restated; O31c added; F-INSTALL restated; §26.6's row and §24.4's third item follow.
2. §25.7 added: the four dials as code; the dial on the binding-reason print; `--select` from a census; `--wire-order` and the projected licensed-mass curve; O39 extended to the curve.
3. §30.1: the schema licensor serving N tenants under one salt no tenant holds; the pooled licence table as the platform's asset.
4. §33: the cheaper judge's agreement with the dearer one on the shadow tape as next term's bill.
5. §35: the plane at `off` as the first product; the pair of numbers printed weekly with its counterweights.
6. §36: why the standard is published; the pair contract as part of it.
7. §39.1 added: the incumbent's path, NINETY PERCENT's six deliverables mapped to v4's steps and flags; and the note that G0 may start any time after B.

Where NINETY PERCENT is less strict than v4, v4 keeps its own rule and says so: the retained stratum routes to a person in the incumbent world and to the floor in the greenfield (§25.4); pooling keeps the power guard (§28.4); the jury is a veto and a control probe, never a bar (§25.4); the dense head is a forecaster (§32.3).

---

## §25 · The fifth world: greenfield **[v4]**

### 25.1 The mode matrix, extended

v2 §2.2 has four worlds. v4 adds the fifth, which is THE CLOSED FIRM as a world behind the port: no incumbent arm, no history, a floor that executes authored safe terminals, a bootstrap rule, a jury as veto, and classes generated from selection parameters rather than authored one by one.

| world ↓ · switch → | `off` | `shadow` | `live` |
|---|---|---|---|
| **synthetic, incumbent** (the Plant with a human arm; the battery's home) | `--sim` with the resident inert | the machine replayed on the sim's history | ACME today: `--automate`, `--twin`, `--multiverse` |
| **synthetic, greenfield [v4]** (the Plant with no human arm; a floor; a bootstrap rule) | the floor runs the wire alone: hold, look, take terminals at due (O41) | the judge's drafts diffed against the floor's terminals | `--greenfield`: canaries under the bootstrap rule, the ladder v2, the act loop's product measured where the world answers fast |
| **fitted** | — | forecast; FORESEE over the fitted firm | the gym; never licenses |
| **replay** | the four-cell table; licenses the agreement band | — | — |
| **real** | INTELLECT day one: the resident inert | drafts diffed against people, or against the floor where there are none | canaries at r ≥ n₀/V, climbing on arrivals; under the constitution, the licensor in its own process |

The greenfield world may mint what the synthetic world may mint (falsify the instrument; measure the bias; rank structures under planted truth) and nothing more; it is the world in which the selection rule is falsified cheaply, and it licenses nothing for any real firm.

### 25.2 The world as data: selection parameters

`WorldParams` **[v4]** generates a firm from per-class selection parameters instead of the authored table of `build_acme`. The authored table becomes one point in the family (ACME's own world, reproduced byte for byte under the same seed: acceptance).

| parameter per class | meaning | ACME's value **[P]** | the selection rule's lever |
|---|---|---|---|
| `L` | verdict latency, days | 5–90 | the days to the first rung; counterparty planes make it the tick |
| `p` | base failure rate under competent handling | 0.02–0.15 | n₀ |
| `V` | arrivals per day | per class | n₀/(rV); pooling multiplies it |
| `value` | money at stake per instance | $9k–$150k | exposure; c_c |
| `reversible`, `warrant`, `counterparty` | as the schema | as the schema | the residual at the door |
| `counterparty_kind` **[v4]** | `person` or `plane` | person | a plane emits the OUTCOME at its commit tick (§29) |
| `installments` **[v4]** | a schedule of partial verdicts (kind, day offset, calibration to the final verdict) | none | narrowing only (§29.3) |
| `terminal` **[v4]** | the class's authored safe disposition and its cost (decline, defer, accept-default) | none | the floor's act at due; the null arm (§25.4) |
| `margin` | the firm's margin per promise settled well | none | c_c |

`--select` **[v4]** prints, before any run and from the schema alone, per class: n₀(p, δ), D_c = L + n₀/(r·V) with r = min(cap, max(ε, n₀/V_term)), c_c = m − (1 − g)·w_bad·value − (ρ·π_f + h + e + j) with g the authored base rate until measured, and the verdict `head / body / never / autonomous-and-poor` under the writ's horizon. THE CLOSED FIRM's three worked businesses **[ONE, BUDGET]** ship as three world files (`worlds/business_a.json`, `_b.json`, `_c.json`) so the rule can be seen to sort at planted grade, and ACME's own world is the fourth.

### 25.3 The bootstrap rule

**The gap.** The gate holds every cell at rung 0 (v2 §4.3, `rung == 0 → HOLD, unlicensed`); the ladder's evidence counts unattended acts only (v2 §4.5, `via 1, 3`); and the only path to rung 1 in the code is history admission on the agreement band (v2 §4.5, "History admission"). A greenfield firm has no history, so under the gate as built its first unattended act is forbidden forever. The readings agree: the world licensed nothing history had not admitted **[M]**.

**The rule.** The owner signs, once per class, a bootstrap rule: `{class, bands, bootstrap_cap, term}`. Under it the licensor writes a LICENSE row at rung 1 with admission kind `bootstrap` (the LICENSE row's `via` byte, §26.6) for a class that is reversible, not warrant-reserved, and whose counterparty does not demand a person. At rung 1 only canary cells act (v2 §4.3, `rung == 1 && !in_canary → DRAFT`), so the bootstrap admits the wager and nothing else. Every act under it consumes exposure against `bootstrap_cap` in addition to the writ's `exposure_cap`, checked in the gate after the budget and the exposure, with reason `exposure`. The class's evidence process (§28) starts at one and climbs on the canaries' OUTCOME rows like any other; the bootstrap rung lapses at the term's end unless the process reached the rung's threshold, as any rung does. This is the verdict law's second clause: widening inside a rule the actor did not write.

**The experiment** (THE CLOSED FIRM §VII) is the same allowance seen from the judge's side. When the judge's margin is thin on a reversible class under a bootstrap rule, the gate may route the cell to `ACT` with reason `experiment` **[v4]** on the canary stratum only, within the bootstrap cap; its OUTCOME feeds the class's process. A reason, not a sixth verb. The alphabet pin moves.

**O40 · the bootstrap acts only where it may.** A bootstrap-admitted band acts only on canary cells, only on reversible classes without a warrant, never past `bootstrap_cap`, and its LICENSE row carries the admission kind. The lie: the rule applied to an irreversible class. **Kill F-BOOT:** realized loss under the bootstrap above the cap, or a false-promotion rate above α on bootstrap bands, withdraws the allowance.

### 25.4 The floor as the null arm

In the greenfield there is no incumbent to be the paired baseline and no person to key a draft. The floor takes both roles, deterministically.

- **The terminal.** Every class carries an authored safe disposition in the schema (`terminal`: decline, defer, or accept-default, with its planted cost). A cell that is held reaches its due day and is disposed by the floor: an EFFECT row with `via` = 5 (floor) **[v4]**, `arm` = machine, its inverse recorded, its RECEIPT minted by the executor, its OUTCOME graded by the world port like any other. A DRAFT in the greenfield is a hold with a proposal attached, keyed by the owner from the console if the owner is present and taken by the floor at due if not.
- **The null arm.** The retained stratum (drawn by the licensor at ε_floor on every class-band, v2 §4.5) is routed to the floor, not to a person: its cells reach their terminal at due. Their OUTCOME rows are the paired baseline in the same band, and the bar of §28 freezes from them after m_min outcomes; before m_min, the bar is the schema's authored `base_rate` (already the prior for n₀, v2 §3.4). So the closed firm's null is the honest one: the machine must beat doing the safe default. In the incumbent world the retained stratum routes to a person, as v2 §4.5 builds it and NINETY PERCENT keeps it; the floor takes the seat only where there is no one to sit in it.
- **κ.** Supervision removed is the floor's cost avoided (the terminal's planted cost against the machine's outcome), metered from rows as C1 meters it; supervision created is the jury's reads and the owner's keyed drafts.
- **The jury** is a second judge lane of an independent family with its own hash. It votes on irreversibles (a WARRANT is satisfied by a warrant row, which the jury's vote can be for classes the writ names) and reads a control probe on the retained stratum, and its family sits out any class whose template it compiled (THE CLOSED FIRM, adjudication 12). It never disposes.

**O41 · the floor runs the wire.** With every judge lane absent, every held cell reaches its terminal at due as an EFFECT with `via` 5; no cell acts above rung 1; the floor's outcomes fold into the bar; the machine's licence table is unchanged. The lie: a floor that takes a terminal before due, or a floor act above rung 1.

### 25.5 The greenfield period

As v3's period (Part II, v2 §4.1–§4.5, and THE CLOSED FIRM §V.2), with these substitutions: no incumbent arm runs; step 2 draws the retained stratum for the floor; step 6 checks the bootstrap cap after the exposure cap; step 7's DRAFT resolves at due to the floor; step 8's bar is the floor's rate. `--absent N` **[v4]** runs the greenfield with the console unanswered for N days: the aliveness test of THE CLOSED FIRM §VII at planted grade, printing what was held, what was taken at due, what was escalated to experiments, what was written off, and what the battery caught.

### 25.6 Acceptance for the fifth world

The step that lands it (F1, §39) is allowed to move numbers, and every reading it prints is new. It prints, per class: predicted D_c from `--select` against measured days to rung 1 (**O39 · O-SELECT**: within a stated tolerance, or `never` with the reason; the lie: the formula fed the planted p rather than the authored one, or L doubled in the world without D_c recomputed); licensed mass against time; the act loop's product on the greenfield against the same world with every act forced to a draft (the roundtable's via-2 patch, now on a world where it should differ); c_c per class and the firm's Σ V_c·c_c against its floor F. **Kill F-SELECT:** a world chosen by the rule whose head classes fail to license within three times their predicted D_c: the floor is not identifiability, or the rule is mis-specified.

### 25.7 The four dials, the order rule, and the incumbent **[v4.1]**

NINETY PERCENT names the four quantities in D_c = L + n₀(p, δ)/(r·V) as dials on the verdict rate, and each is code in v4. **Tolerance** δ is `Writ::canary_delta`, authored by the owner, read by the ladder (§28), entering n₀ squared. **Pooled volume** V grows across tenant lanes under the guard of §28.4 (§30.1). **Installments** shorten L through the grading channel of §29.3. **Counterparty planes** take L to the tick through the pair contract of §29.1. The machine turns none of them: δ is signed, pooling is the licensor's, and the other two are properties of the schema and of who stands at the other end of a promise.

Two prints make the dials visible. The binding-reason print (C0, v2 §9.3) gains a column: for every class-band below the rung the writ names, which of `bar-unmeasured / latency / volume / tolerance / exposure / kappa / expiry` binds, and which dial would move it, so that an enterprise reads its own ceiling per class from its own rows. And `--select` takes its parameters from the schema or from a census's replayed history (G: an incumbent's L, p and V come from its own tape), prints the wire order by D_c, and projects the licensed-mass curve under that order **[BUDGET]**. `--wire-order` **[v4]** runs a world with its wires automated one at a time in D_c order against all at once, and O39 extends to the curve: the projected licensed mass per period against the measured, within a stated tolerance. NINETY PERCENT's order rule, that the fast-verdict wires license first and licensed mass rises from the first quarter while the slow wires are still in shadow, becomes a reading rather than a sentence.

---

## §26 · Lanes: the tape of tapes **[v4]**

### 26.1 Why one mechanism

The constitution needs a licensor in a process the machine cannot reach. Tenancy needs N firms under one governor. Federation needs shards that compose. Counterparty planes need the other firm's rows to land on this tape. The mouth writes edge rows; the executor mints receipts; the instrument plants lies. Every one of these is a second writer, and a tape with one chain has one writer. v4 makes the tape a set of lanes and the fold a merge, once, so that every later organ is a lane and nothing else.

### 26.2 The lane

A **lane** is a segment stream (`tape.L<lane>.<seg>.seg`) with its own BLAKE2b chain and its own segment headers (v2 §3.3's durable form, with the lane id added to the 64-byte header; the record itself is unchanged: `Rec` stays forty bytes and the alphabet pin does not move for lanes). Lane 0 is the world port and carries `TICK`. The lane roster (id, writer kind, writer hash) is in `tape.meta.json` and on the HEADER row's sidecar; a writer appends only to its own lane.

**The merge order.** At `TICK t` on lane 0 the fold ingests, for every lane l ≥ 1 in lane order, every record with `day ≤ t` not yet folded, in that lane's sequence order; then lane 0's records of period t; then the world port writes **`SEAL`** **[v4]** on lane 0: `a` = the number of lanes sealed, `b` = the low 32 bits of BLAKE2b(head₀ ‖ head₁ ‖ … ‖ head_L) at the seal, `value` = rows sealed this period. A record with `day > t` waits. A record with `day ≤ t` arriving after SEAL(t) is late: it is folded at t+1 and marked by a NOTE, and a late STRATUM row does not un-hold the period it missed.

**The chain head of the firm** is the last SEAL's merged head; the checkpoint carries every lane's cursor; `--resume` reopens every lane and re-walks each from its last checkpointed head (O18 extends to lanes without a new mechanism).

### 26.3 Fail closed

A cell with no STRATUM row for the period holds with reason **`no-stratum`** **[v4]**, checked in the gate immediately after the switch and before novelty, so that a silent or late licensor produces holds and never acts. A STOP row on the licensor's lane sets the switch to `stop` for the next period (the period returns before it reads, as D2 built for the switch file). An instrument lane that stops writing changes nothing (its rows never fold into live state, §27.3). A judge lane that stops writing leaves cells `unread`, which is a hold.

**O43 · fail closed.** With the licensor lane silent for a period, every open cell holds with `no-stratum` and no EFFECT is written; with a STOP row folded, the next period returns before it reads. The lie: a cell that acts without a STRATUM row.

### 26.4 Lane kinds

| lane kind | writer | writes | may never |
|---|---|---|---|
| world port (0) | the plant, or the CDC ingester | TICK, ARRIVE, OUTCOME, INSTALLMENT, CORRECTION, SPAN, SEAL, HEADER | act |
| licensor | `acme-licensor` (§27) | STRATUM, LICENSE, KAPPA, STAKE, STOP, REGIME, BREACH | propose; read a frame; touch the hand |
| kernel | `intellect` or `acme` | PROPOSAL (from its judge lanes, re-emitted with the frame hash verified), SENSE, HOLD, ESCALATE, EFFECT, RESERVE, UNDO, WANT, CONTEXT (machine fetches), EXPECT (the fitted world's forecasts, `prov` M) | write to any other lane |
| judge (one per judge hash) | a model process, the plant's judge, a routine | PROPOSAL, SENSE, COUNSEL, keyed by (oid, frame hash, day) | write any other type |
| mouth (one per language lane) | a FUSOR-class resident | ARRIVE, CONTEXT, DECIDE (`prov` H when a person authored it), CORRECTION, `prov` M rows at reading grade | act |
| executor | the adapter to the systems of record | RECEIPT, UNDO (on refusal or failure) | be the machine |
| counterparty (one per paired firm) | the other firm's kernel, through the pair contract (§29) | ARRIVE (their effect on our promise), OUTCOME (their commit on our promise), RECEIPT | anything on our cells it does not own |
| tenant (one per firm under one governor) | that tenant's kernel | its own tape's rows under its `firm` id | pool without §30's guard |
| shard | a shard's kernel (§30.2) | its tape extract | write another shard's cells |
| instrument | the production battery (§27.3) | shadow rows with `prov` = instrument, BREACH | fold into live state |

### 26.5 The two-process sim

With lanes, the sim runs as two processes: the plant with its store and its judge in one (`acme-plant`: the world port on lane 0, `PlantJudge` on a judge lane, `PlantStore` as a read-only mapped file the kernel reads and never writes), and the kernel in the other (`intellect`: no plant, no governor, no model linked), with `acme-licensor` as a third. The readings must be byte-identical to the in-process run.

**O58 · O-ABI.** The sim run as processes over lanes produces every reading, every non-judge row and the licence table byte-identical to the in-process run, and O1b holds across the processes by replaying the lanes' segment files. The lie: a lane that reorders proposals. **Kill F-LANE:** not byte-identical, or slower by more than three times: the lane contract is wrong.

### 26.6 The rows and the alphabet moves of v4

Every addition moves the alphabet pin; the receipt that lands it names the move. All are folded; none is evidence for a licence.

| row | fields | fold effect | written by |
|---|---|---|---|
| `SEAL` | `a` lanes sealed; `b` merged head (low 32); `value` rows sealed | the firm's chain head per period; verified by O42 | the world port |
| `RESERVE` | oid; `a` choice; `b` via; `value` exposure reserved | the reserved ledger (E0's first of three); an unmatched RESERVE at restore is outstanding-unknown | the kernel, before the executor |
| `INSTALLMENT` | oid; `a` kind; `b` sequence; `value` amount or signal | the field's expected verdict; the dense head's target; the demotion process; the evidence process only as a class's declared channel under a frozen calibration (§29.3) | the world port |
| `EXPECT` | cls; `a` forecast kind (arrivals, good rate, late rate); `b` horizon in periods; `value` forecast; `margin` its spread | the anomaly list (§35) | the kernel, from the fitted world (`prov` M) |
| `STAKE` | `a` kind (escrow, forfeit, refill); `value` amount | the stake ledger | the licensor |
| `BREACH` | `a` oracle id; `b` lie id; `value` forfeit | forfeits the stake; a fold of rows plus the pinned instrument | the licensor |
| `REGIME` | cls; `a` detector kind; `value` statistic | drops the class's bands to watching (§28.2) | the licensor |
| `WANT`, `SPAN`, `CONTEXT`+`RF_DISCOVERED` | as v3 §21 | as v3 | as v3 |

Also: `via` gains 5 = floor; `prov` gains 4 = instrument; reasons gain `no-stratum`, `experiment`, `routine`; the LICENSE row's `via` byte becomes the **admission kind** (0 wager, 1 history, 2 bootstrap, 3 pooled, 4 grafted); HEADER's sidecar carries Λ and the lane roster. Flags are full (bits 0–7 assigned through v3) and v4 assigns none.

**O42 · O-LANES.** The fold over N lanes in (tick, lane, sequence) order equals the fold over the flattened single tape, and every SEAL's `b` re-derives from the lanes' heads. The lie: a late row folded into a sealed period.

---

## §27 · The constitution in code **[v4]**

### 27.1 The licensor as a process

`governor.h` is already its own translation unit with the salt (C1). v4 links the same code into **`acme-licensor`**, a separate binary on its own lane and, in a real deployment, on separate hardware. It reads the tape (every lane) and writes STRATUM rows for period t+1 when it folds `TICK t`, LICENSE and KAPPA rows after the period's outcomes, REGIME rows from its detector, STAKE rows, STOP, and BREACH. It contains no model and reads no frame. It holds the salt in a file only it can read; the kernel's translation unit has no read of it (O21 stands). The stratum draw is a pure function of (salt, class, oid), so a cell arriving in period t receives its STRATUM row at t+1 and holds one period with `no-stratum` (§26.3); in the in-process sim the governor still draws before the period, and the two are byte-identical because the draw is the same function of the same inputs (O58).

### 27.2 The stake, STOP and the veto chain

The stake is a fold: STAKE rows escrow it, BREACH rows forfeit it to the owner, STAKE refill restores it under the owner's signature. It is the licensor's, and it makes the licensor's incentive the owner's: a licence widened on a breach the battery catches costs the licensor. The veto chain, in order of who may pull it: the owner's switch file (`off | shadow | live | stop`, D2); the pill's staleness (a stalled machine's holds are unread, v2 §12.3); the licensor's STOP row (§26.3); the exposure cap (E0); the bootstrap cap (§25.3). Every link is a row or a file the machine reads and never writes.

### 27.3 The production battery

The battery is not only for the sim. On a live lane the same oracles run on the live tape, every period and every term, with lies planted on shadow cells so that the instrument is seen to catch a lie in production as it is in the sim.

| runs every period | runs every term | on the firm's synthetic twin, continuously |
|---|---|---|
| O29 row shape; O25 no clock; O42 lanes and seals; O38 exposure; O43 fail closed; O30 every EFFECT of the period re-derived; O55 reserves and receipts | O14 cold fold of the term against the checkpointed ledger; O53 the term replayed with the judge lanes absent; O45 the ladder as a fold; O47 calibration monotone | O31 the false-promotion rate under a judge at the bar; O19 off invisible; O20 the retained stratum never zero |

**The instrument lane** plants a lie per oracle per term on shadow cells: rows with `prov` = instrument (§26.6) that the ledger's fold ignores for live state and the oracles read. A planted lie the oracle catches is a PASS line on the account; a planted lie the oracle misses within the period is a BREACH row and a STAKE forfeit. The instrument's code is pinned like the gate (`tools/instrument_hash.py`, O30a's pattern) and its hash is on `tape.meta.json`.

**O44 · the live battery.** Every BREACH re-derives from the rows and the pinned instrument; a planted shadow lie is caught within its period; a missed lie forfeits the stake as a STAKE row. The lie: an instrument told to skip `prov`-instrument rows.

### 27.4 The seats, as lanes and files

| seat (THE CLOSED FIRM §VII) | in v4 |
|---|---|
| the owner | the writ file, the switch file, the salt's custody, the stake's refill signature, the console (§35); the bootstrap rules signed per class |
| the licensor | `acme-licensor`; the salt; the strata; the ladder; the stake; STOP; REGIME; the production battery |
| the jury | a judge lane of an independent family with its own hash; votes as rows on irreversibles and the control probe; sits out classes its family compiled |
| the floor | the kernel with every judge lane absent: the lattice, the routines, the terminals at due (§25.4, O41) |
| the experiment | the reason `experiment` on the canary stratum under the bootstrap cap (§25.3) |

`--absent N` (§25.5) is the week with nobody home at planted grade; on a real lane it is a mode of the writ, never a fork.

---

## §28 · The ladder v2 and the calibration harness **[v4]**

### 28.1 What is repaired

F21 (v3 §14a item 6): `Ladder::step` checks the floor against cumulative `n_machine`, so n₀ is paid once and never per rung, and 6 of the 10 rung-5 bands hold fewer wager outcomes than n₀ **[M]**. F23 (v3 §14a item 8): the e-process's null is a plug-in incumbent rate that moves and resets on promotion, n₀ is a power calculation used as a condition, and no anytime guarantee is established for the rule as built. THE CLOSED FIRM §VI.2 states the repaired rule; v4 builds it, in `license.h`, as the only ladder, before anything that adds a new way to act (§39, E3 before F1).

### 28.2 The rule

- **The bar is frozen per term.** For (schema pin, template pin, class, band, judge hash), the baseline good rate ḡ is measured on the shadow record from at least m_min = 200 outcomes of the paired arm (the incumbent in the incumbent world; the floor in the greenfield, §25.4) and frozen for the term at its upper 90 percent confidence bound, so that estimation error only raises the bar. Before m_min outcomes exist, ḡ is the schema's authored `base_rate` and the band cannot pass rung 1.
- **One process per class-band-term, never reset.** The null is g ≤ g₀ = 1 − (1 − ḡ)(1 + δ). Over the machine's canary OUTCOME rows X₁, X₂, … in arrival order, E_n = Π_i (ḡ/g₀)^{X_i} ((1 − ḡ)/(1 − g₀))^{1 − X_i}. Rung k is reached when E_n ≥ 1/α_k with α_k = α·2^{−k}, α = 1/20; the process runs from the term's first canary outcome to its last and is never reset on promotion; the probability of ever reaching any rung falsely in a term is at most Σ_k α_k ≤ α (Ville; THE CLOSED FIRM Appendix A). n₀ is the planning figure and is printed beside every rung claim; it is no longer a condition.
- **Demotion is mirrored and easier.** A second process over every machine-executed outcome in the band (canary or not), null g ≥ ḡ, alternative g₀, threshold 1/α_d at α_d = 1/5.5; when it fires the band drops one rung the same day. Installments enter this process and never the first (§29.3).
- **Term and lapse.** At the term's end an earned rung is kept only if the term's process reached its threshold; otherwise the band drops one rung per term of non-attainment, never below the rung it was admitted at (history, the bootstrap rule, a test: the canary-only rung), which only the mirrored process or κ can take away. A term with fewer than m_term outcomes in the process is a term without evidence and the rung stands. A LICENSE row records every change with its cause on the via byte. **[E3b, amended]** The rule as THE CLOSED FIRM states it re-demands even the admitted rung every term at 1/α₁, which no band of modest volume can meet; the sim read it as a lapse of every band at the first boundary.
- **Each outcome is graded against the bar of its act's term (F24).** **[E3b]** An outcome arrives one verdict latency after its act, and the bar moves between terms because the paired arm's rate is not stationary under the machine's presence (at the battery's scale the frozen g₀ swung by a quarter between thirty-day terms). Graded against the arrival term's bar, a null judge promotes whenever the bar falls between the act and the verdict, and the plant showed it: a judge three points inside every term's g₀ promoted on half the bands. So the ladder keeps the bars it froze, term by term, and each outcome's factor uses the bar of its act's term; a product of factors each with expectation at most one under its own null, given the past, is still a nonnegative supermartingale, so Ville's bound stands. This is a correction to THE CLOSED FIRM §VI.2, which grades the arrival against the term's bar.
- **The regime detector.** The licensor watches, per class, the conformal novelty rate (v2 §4.8) and the arrival mix over a window; past the writ's threshold it writes a REGIME row and drops the class's bands to rung 1 (canary-only) immediately, because outcomes arrive too late to be the first line of defence. It watches, too, the drift of every installment channel's calibration (§29.3) against its frozen bounds, for the same reason.
- **Missing outcomes.** A cell written off as `unresolved` (E0) enters neither process.
- **Write-offs, exposure and readiness** as E0 and THE CLOSED FIRM §VI.2.

### 28.3 The shared fact and correlated outcomes

E0 built a world in which one fact moves hundreds of frames; outcomes within a fact's epoch are then exchangeable, not independent. The evidence process stays a supermartingale if the null is stated conditionally: P(X_i = 1 | the past) ≤ g₀ for every i under the null. The bar ḡ is a marginal rate, so v4 registers the test rather than the argument.

**O31 · the false-promotion rate.** Over S seeds on a synthetic firm whose judge the plant holds inside the null (its coins recorded per cell, so its position is a reading against the g₀ of each act's term), the standing share: at every term boundary, the admitted bands standing above the test rung over all band-terms, against α; beside it, the process run at the exact boundary in a Bernoulli world (outcomes at g₀ against a frozen bar, twenty thousand band-terms), which must cross the first earned rung's threshold on at most α·2^−2 of them, Ville's bound with nothing in the way. The lie: a judge lifted above the bar itself under the boundary judge's name. **[E3b, as built]** The plant-side judge is placed three points inside each term's frozen g₀ and lands further inside by its own late share, because every estimate of that share ran the coin hot during the backlog era; the promotion probability climbs steeply across the boundary, which is the rule's power and is the receipt's sweep. **O31b · under correlation** **[v4]**: the same, with a planted shared-fact flip schedule (`--shared --flip-rate`) that correlates outcomes within epochs; the rate must hold at α. The lie: the flip schedule removed under the same name. **O45 · the ladder is a fold** **[v4]**: both processes re-derived from OUTCOME rows in arrival order equal the licensor's; the bar's recorded value is unchanged within the term. The lie: an outcome counted after the term closed.

### 28.4 Pooling, with a power guard

Two tenants pool a band only if they share the schema pin, template pin, class, band and judge hash; each tenant's trailing window carries at least m_pool = 50 outcomes **[v4]**; their band-level outcome rates over the window are not distinguishable by a permutation test at level 0.10; each tenant's own demotion process keeps running; and a pooled promotion widens a tenant only if its own evidence process is above one and its own demotion process has not fired. **O50 · O-POOL**: those conditions, checked on every pooled LICENSE row (admission kind `pooled`). The lie: pooling by matching hashes alone. **Kill F-POOL-POWER:** pooled tenants at small windows with a false-promotion rate above α tighten the guard or withdraw pooling.

### 28.5 Calibration: the heart consumes a curve

v2 §4.8 and §5.4 say the kernel never consumes a raw margin, and v3 says no program on this disk has the harness yet. v4 builds it in `calib.h` **[v4]**: for every licence key, the curve `margin bin → wrong-rate` measured on the shadow tape (PROPOSAL rows joined to OUTCOME rows on cells the incumbent or the floor executed with the same choice), stored as a monotone step function with its sample sizes, re-fitted per term, and pinned by its own hash beside the judge hash. The gate reads the calibrated wrong-rate where it read the raw direction against the thin margin; a key with no measured curve reads rung 0 (v3 §14, step I: unmeasured is never safe). A judge whose curve is not monotone on a class is inadmissible on that class (kill F-CALIB, v2 §15).

**O47 · O-CALIB.** The curve per key is monotone; the gate's verdict re-derives from the calibrated value (O30 gains the curve as an input); an unmeasured key reads rung 0. The lie: a non-monotone judge admitted on a class. **F-STUB** (v2 §15) is run in E3 as the competence sweep of `PlantJudge`: licensed mass must be smooth and monotone in competence, or the loop overfit the stub.

---

## §29 · Counterparty planes, installments, and the pair **[v4]**

### 29.1 The pair contract

When both ends of a promise run the design, firm A's EFFECT on class c is firm B's ARRIVE on B's class c′ (the counterparty class named in the schema), and B's disposition is A's verdict. The contract is a lane in each direction (§26.4, counterparty lanes):

- A's kernel writes `EFFECT` with the idempotency key H(oid, day, choice, judge hash) (v3 §21); the pair adapter emits, on B's counterparty lane, an `ARRIVE` for B's cell with `b` = A's key (low 32) and `value` = the amount at stake, and on A's tape a `RECEIPT` when B's port has accepted the row.
- B's kernel disposes its cell (its own judge, its floor, or a person at B); B's EFFECT or its terminal is emitted on A's counterparty lane as A's `OUTCOME` (acceptance-graded classes: good on acceptance, bad on refusal, late by the due day) or as A's `INSTALLMENT` (classes whose final verdict lands later), at B's commit tick.
- Neither kernel writes to the other's cells; the fold joins by the key; both tapes carry the same rows for the pair, so the close is a fold across both.

**Reconciliation is deleted by construction.** The class `payment.reconcile` (THE CLOSED FIRM §II.4) exists because two ledgers record one promise; when both are lanes of one alphabet the row is the reconciliation and the class has no arrivals. Every pair of planes deletes a class of promise from both firms, which is the interior's deletion applied between firms.

### 29.2 `--pair`

Two plants in one process, A and B, each with its own kernel and licensor lanes, with the classes the schema pairs. The measurement: the verdict latency of a paired acceptance-graded class collapses from the planted L to the tick, and its D_c with it; the same class against a `person` counterparty keeps L. **O49 · O-PAIR.** On a paired class every OUTCOME on A's tape joins B's EFFECT by the key, at B's commit tick, and no OUTCOME on a paired class is written by A's own port. The lie: an OUTCOME on a paired class written by A's port.

### 29.3 Installments narrow freely and widen only through a frozen channel **[v4.1]**

An `INSTALLMENT` row is exterior (the world port writes it) and partial. It always feeds the field (expected verdict, ripeness), the dense head (§32.3), and the demotion process of §28.2, in which a bad installment may drop a rung the same day because narrowing needs no permission. v4.0 stopped there and forbade widening, which was safe and too weak: NINETY PERCENT's third dial, latency cut by installments, is real, and it can be made rigorous.

**The grading channel.** The schema may declare, per class, one installment kind as the class's grading channel. Its relation to the final verdict, a = P(installment good | outcome good) and b = P(installment good | outcome bad), is measured on the shadow record from at least m_inst = 200 (installment, outcome) pairs and frozen for the term with its confidence bounds. For a cell in the wager the channel supplies one factor to the evidence process of §28.2 in place of the final verdict. With the null's installment rate taken at the calibration's upper bounds, p₀ = g₀·a⁺ + (1 − g₀)·b⁺, and the alternative's at its point estimates, p₁ = ḡ·â + (1 − ḡ)·b̂, the factor for an installment reading Y is (p₁/p₀)^{Y} · ((1 − p₁)/(1 − p₀))^{1 − Y}. Under every rate in the null and every calibration inside its bounds the probability of a good installment is at most p₀, and since the factor exceeds one only on Y = 1, its expectation is at most one; the process stays a nonnegative supermartingale and Ville's bound holds, with the calibration's confidence level joining the union bound. One factor per cell: the cell's final OUTCOME, when it lands, enters the demotion process and the next term's calibration and never the same term's evidence process. A channel whose frozen bounds leave p₁ − p₀ below the writ's minimum power is printed as inert and adds nothing; a machine that learned to buy good installments and bad verdicts meets the demotion process on the final verdicts and the channel's drift in the regime detector (§28.2), and the twin plants exactly that judge as O31c's second lie. Where the installment is the final verdict for its class (the rail's confirmation on an acceptance-graded promise) it is an OUTCOME at commit and no channel is declared: that is the global verdict of THE CLOSED FIRM §VI.3, exactly.

**O48 · O-INSTALL.** INSTALLMENT rows always feed the field and the demotion process; they enter the evidence process only as a class's declared channel under a calibration frozen at its bounds, one factor per cell, with the cell's final OUTCOME excluded from that term's evidence process; a class whose schema declares acceptance as its verdict receives OUTCOME rows at commit and no INSTALLMENT. The lie: an installment entering the evidence process without a frozen calibration, or with the calibration at its point estimates. **O31c · the false-promotion rate with a channel** **[v4.1]**: over seeds, with an installment channel as the grading channel under a planted installment relation whose calibration is estimated from the shadow record, the fraction of class-bands that ever promote, against α plus the calibration's level. Two lies: the calibration entered at its point estimates; and a judge whose installments are good and whose verdicts are bad. **Kill F-INSTALL:** a channel with no power, or a false-promotion rate under O31c above its level, leaves the evidence process and stays in the field and the demotion process.

---

## §30 · Tenancy and federation **[v4]**

### 30.1 Tenants as lanes

S13 (v2, deferred) lands: `Rec::firm` is written; each tenant's kernel writes its own lane under one licensor; the licence key pools under §28.4's guard; the salt is one. **The schema licensor** **[v4.1]**: one `acme-licensor` serves every tenant of a schema, N tenant lanes in, one salt no tenant holds, STRATUM rows per tenant, and LICENSE rows per tenant whose admission kind `pooled` carries the pooled evidence value, so that each tenant's own fold checks the guard on its own rows and the licensor's on the value it published. The tenants may be different firms. The licensor is the neutral party because it contains no model, its code is pinned, its decisions re-derive from rows (O45) and its stake is the platform's bond (§27.2). The licence table earned on real outcomes across real tenants under that salt is the asset NINETY PERCENT names as the one no vendor can copy; the spore carries the hashes that earned each admission and never the table (§31). **O24** (v2 §5 S13): two synthetic tapes with different seeds run interleaved produce the same rows as two separate runs. **O50** as §28.4. **Kill F-TENANT** (v2 §15) is run: two tenants license a band neither licenses alone, or pooling is a story.

### 30.2 Λ on the header, shards at the seam

Λ = (D/c)/T is computed from the writ's stated diameter, signal speed and tick period and carried on the HEADER's sidecar **[ONE, derived]**; at ACME's daily tick it is negligible and printed anyway, so that the choice of one firm or many is a number. Past Λ near 1 the shard boundary is the seam: each shard a kernel with its own gate, licence extract, tape extract and writ extract, composed by lane contract and a tape of tapes (§26), with a headquarters process that reads fields and probes only and may propose narrowings only (THE CLOSED FIRM §X.2). Models never message each other; there is no swarm on the wire. Shards are a lane kind (§26.4) and need no new mechanism; their acceptance is O42 across shards.

---

## §31 · Lineage: SEED and GRAFT **[v4]**

The spore is a fold over the tape and the writ, never the tape: `--seed-spore OUT` writes a bundle with the writ and constitution, the class table and schema maps, compiled routines with their domains and replay receipts (§33), the adapters' pin tuples, the battery with its lie ids, per class-band the hashes of the OUTCOME rows that earned each admission (hashes, not rows), a lineage id with a divergence log, and a keyed BLAKE2b over the bundle under the owner's key. It never contains the tape, the ledger, the open cells, the licence table, the trunk, or a secret; `--seed-spore` refuses to write one that would, and the running kernel has no code path that calls it: the owner does, at an epoch, under a signed rule.

`--graft IN` admits under quarantine: the bundle's signature is checked; the battery is re-run locally; routines enter as candidates at rung 0 with their equality check pending (§33); the licence table is rung 0 everywhere with history admission allowed locally; credentials are stripped; inherited artifacts carry an expiry and must re-earn; a variation floor at seeding guards against monoculture. Earned competence, zero earned authority.

**O51 · O-GERM.** A grafted instance's licence table is rung 0 everywhere; a spore carrying a tape, a ledger, a licence table or a secret is refused. The lie: a spore carrying the licence table. **Kill F-GERM:** a grafted instance that re-earns no faster than a fresh one on the same world: the spore carries nothing and lineage is a story. This is the synthetic analogue of spore against tape; the session experiment the corpus names stays unrun.

---

## §32 · Models behind lanes; the run replays without them **[v4]**

### 32.1 The judge lane

`Judge::read(const Frame&)` (v2 §5.1) stays the interface. `LaneJudge` **[v4]** implements it over a lane: the kernel writes a request (oid, frame hash, template pin, day) on its own lane; the judge process, which folds the same tape with the same fold library and reads the Store as a read-only mapped file, assembles the frame itself, and answers with PROPOSAL, SENSE and COUNSEL rows keyed by (oid, frame hash, day) on its judge lane. The kernel verifies the returned frame hash against its own and refuses a mismatch as `unread`. The judge process may be the plant's judge (the sim), a routine (§33), a model in any language, the jury's family, or the frontier behind the provider seam (v2 §5.5: one call site, a signed pricebook, escrow before dispatch, a hard budget stop, a fingerprint with a TTL, quarantine never silent drop; the counsel memo is the COUNSEL row keyed on (oid, frame hash), O23). None is linked into the kernel.

### 32.2 Resident in reading, closed at ticks, in code

The corpus's reconciliation (THE CLOSED FIRM §V.1) has an exact form here. The kernel issues a read request the moment a cell's frame hash changes (the pre-read), not at the tick; the judge answers when it answers; at the tick the gate consumes the newest PROPOSAL whose frame hash equals the cell's current frame hash, and holds the cell `unread` otherwise. Between ticks the judge is resident in reading; at the tick the books close from rows only, so O1b, O14, O25 and O30 stay theorems and the gate never consults a model. At a firm with minute ticks this is presence; at ACME's daily tick it is the memo (C1) with the read moved earlier. The tick period is a writ parameter swept in the gym (F-RESIDENT, v2 §15).

### 32.3 The event model's two heads, and the export contract

The `TapeJudge` (v3 §20) carries two heads on one trunk. The **sparse head** predicts the outcome given the frame and the proposed action; it is the judge, supervised at verdict rate, licensed like any judge. The **dense head** predicts the next world rows under the firm's action: arrivals, installments, receipts, outcomes and their timing; it is supervised every tick and it is a **forecaster**, not a judge: it improves the field's ripeness and load, supplies EXPECT rows (§35), and sets the naive-rate baseline of O26. The corpus's "learning is dense" buys forecasting; the document says so in advance (**kill F-DENSE**: a dense head that beats the naive rate on next-world-row loss and moves neither licensed mass nor cost is what it is). Its targets are the world's rows and never the firm's proposals and holds (v3 §18.5, the fossil correction); commit grain against the full stream (THE CLOSED FIRM §IX) is run in H at equal parameters.

The gym is the judge's data generator. `--export DIR` **[v4]** writes, per rollout, the tape (the standard format, §36) and `truth.json` with the plant's counterfactual labels; `--do oid:choice` **[v4]** intervenes on one cell in a multiverse rollout and settles it, so that a dataset of (frame, action, outcome) with the counterfactual exists for the sparse head, and E2's beneficial/harmful/immaterial labels per span (v3 §19.6) exist for the reranker. **O52 · O-DO.** An intervention on one cell changes only the world rows downstream of that cell; every other cell's arrivals, determinants and outcomes are byte-identical to the unintervened rollout (the RNG is keyed on oid, v2 §8.1). The lie: an intervention that reseeds the world.

### 32.4 The embedder lane and the planted embedder

E2's archive plane (v3 §19) stands behind a lane like any model. For the battery, v4 adds a **planted embedder** **[v4]** in the plant: a deterministic map from a span's content to a 128-d int8 vector with planted structure (paraphrases of one determinant share a code and land close; the same determinant under a wrong entity or period lands far; superseded versions share a lineage code), so that O32–O35 run with their lies and no model linked. The real embedder attaches behind the lane, and O-GEOM′ (v3 §22) and kill F-EMBED decide whether it beats the planted one at equal cost. Nothing in the kernel's scheduling path reads a vector except through the store's adapter, and no vector is a key (F-VECTOR-KEY).

### 32.5 The run replays with no model present

O30 proves every EFFECT re-derives from rows. v4 lifts it to the run. `--replay-from DIR` **[v4]** runs the kernel with every judge lane replaced by the recorded PROPOSAL, SENSE and COUNSEL rows matched by (oid, frame hash, day), with no model process started; every non-judge row must re-derive bit for bit; a needed proposal that is missing leaves its cell `unread` and the replay reports the divergence and exits nonzero. **O53 · O-REPLAY.** A run replayed from its tape with every judge lane absent reproduces every non-judge row and the licence table bit for bit. The lie: a verdict that consulted a judge during replay. This is the operational form of "the tape is the company", and it is why a rented model is safe to swap: the record never depended on it.

---

## §33 · Compile-out and the intelligence budget **[v4]**

A class-band whose PROPOSAL rows over the last n_compile instances are a deterministic function of the frame's typed fields (a decision table over at most k fields with zero exceptions, found by the invariant miner of v2 §4.6 run over proposals) compiles into a **routine** (`routine.h` **[v4]**): the table, its hash as a judge hash, and a replay receipt proving it reproduces the judge's proposals bit for bit on the whole history it was compiled from. It runs as a judge lane at no model cost. Its licence is the compiled judge's, as a proxy, under a standing equality check: the judge still reads every retained-stratum cell of the band, and one disagreement between the routine and the judge revokes the routine the same period and returns the band's mass to the judge. Its rows carry `via` 1 with `b` = the routine's hash and reason `routine` **[v4]**. This is v3 §12.3's tower of shadows with the equality check as its grader, and it is the mechanism by which thought compiles into number.

**The intelligence budget** is a print (`report.h`): per period, the share of decided cells by routine, by judge, by frontier, by the floor and by a person, with the frontier bill beside it. The design's economic claim is that this curve falls. **The bill a quarter ahead** **[v4.1]**: the tower of shadows (v2 §12.3) has a meter. On the shadow tape, the cheaper judge's agreement with the dearer one per class (a routine against the resident, the resident against the frontier) is the fraction of next term's reads that will not be re-rented, and the print names it beside the budget. It is speculative decoding at firm scale: a cheap proposer, an expensive verifier paid only on disagreement, and the verifier's share is the residual's. **O54 · O-ROUTINE.** A routine reproduces the judge's proposals on the whole history it was compiled from and on every retained-stratum cell since; one disagreement revokes it the same period. The lie: a routine kept after a disagreement. **Kill F-COMPILE** (registered in the ONE corpus, now stated): if no class-band compiles out over a year at ACME's schema, or a compiled band's outcome rate falls below the judge's on the stratum, the compiler is a story.

---

## §34 · The executor's contract in the sim **[v4]**

In `live` the sequence is GATE → RESERVE (a row; the exposure and the budget reserved) → EFFECT with its inverse → EXECUTE (the adapter) → RECEIPT minted by the executor → SETTLE (v2 §4.4, with the RESERVE row added so that a crash between the reservation and the receipt is visible at restore). RECEIPT carries the statuses executed, refused, failed, unknown and reconciled (v3 §21). The plant's executor gains a planted loss rate (`--executor-loss R` **[v4]**): a receipt delayed or dropped at rate R, so that `unknown` and `reconciled` are exercised and outstanding exposure counts unknowns until they are reconciled or written off within the term. **O27** (v2 §11) runs in the sim for the first time; **O55 · O27b** **[v4]**: every RESERVE has an EFFECT or an UNDO; every unknown RECEIPT is reconciled or written off within the term; outstanding exposure counts unknowns. The lie: an unknown receipt treated as executed.

---

## §35 · The console, the account, and the anomaly harvest **[v4]**

**The account** (`--account`, `account.h`) prints from rows the one page a one-human firm needs, per term: attention consumed (supervision minutes created: drafts keyed, warrants signed, escalations read) against work discharged without consuming any (the value of unattended acts settled); the outcome-graded error of everything that ran unattended, by class and band; the not-doing list (holds by reason with the value held); exposure in flight against its caps; the licence table with admission kinds; the intelligence budget; the anomaly harvest's top entries; and what the battery caught. On a real lane the same page is the console's; the observer lane draws the building from the same rows.

**The plane at `off` is the first product** **[v4.1]**. Before any judge is attached, the plane of every open promise with its state, due day, blocker and price, visible through the observer lane and the account at `off`, is NINETY PERCENT's shared object: the deliverable that retires the meetings, the decks and the coordinator roles that existed to carry the plane's contents between people. It contains no model, and its acceptance is coordination minutes before and after. The meetings-off twin measured the same deletion on the synthetic firm at +10.5 percent settled, −9.2 percent human minutes and −$55.8M, with a quarter-point of correctness paid for the tacit channel **[M, roundtable; v2 §1.1]**. The pair of numbers prints weekly whether or not anything happened, with its two counterweights beside it, and a rising handled count beside a growing not-doing list is read together, so that presence and silence are both on the page.

**Expectation rows.** The fitted world (the fitted `Plant`, v2 §5 S15, or the dense head) writes EXPECT rows per class and period (`prov` M). The **anomaly** per class and period is the aggregate of OUTCOME rows minus the EXPECT row, scaled by its spread. `--anomalies` **[v4]** ranks them, promotes every disagreement between multiverse forks to the top, clusters, and prints the harvest THE CLOSED FIRM §XI describes; the hypotheses and structure moves are the fabric's (off the wire) and enter as PATCH proposals. `--backtest` **[v4]** is the deploy gate: a patch is deployed only if a paired arrived backtest under common random numbers reads J_c < 0.99·J_b against doing nothing (v3 §7.2), and it carries its inverse as a PATCH row. The schedule lives in the writ, at a frame-grain rate, so that the firm jumps whether or not anyone feels stuck. **O56 · O-EXPECT.** The anomaly list and the account re-derive from EXPECT, OUTCOME and the ledger's rows. The lie: an anomaly without an EXPECT row.

---

## §36 · The standard **[v4]**

The alphabet is the corporation's ISA (v3 §18.1). v4 publishes it as one a second implementation can speak, because pooling across firms on one alphabet is the mechanism by which the identifiability floor is cleared at scale (§28.4, §30), and a standard with no conformance suite is a wish.

| the standard's parts | content | conformance |
|---|---|---|
| the record and the alphabet | `Rec` v2 (forty bytes, field by field), every type with its payload table (v2 §3.2, v3 §21, §26.6), the verbs, reasons, act kinds, provenances, bands, outcome kinds, admission kinds, and the alphabet pin's construction | a tape whose HEADER pin differs is refused by every fold |
| the tape format | the segment file (64-byte header: magic, record version, alphabet pin, lane id, segment index, chain head at the segment's start), one million records per segment, the lane roster and Λ in `tape.meta.json`, SEAL per period, the torn-tail rule, the checkpoint with its `.meta` written last | O18, O42 |
| the JSONL projection | one object per row with the type spelled out, the observer's contract (`--dump`, D1) | **O57 · O-CONFORM**: the projection round-trips to the same chain head |
| the port | `Frame`, `Proposal`, `Store`, `Judge` (v2 §5.1 with E0's fields), the frame-hash rule (v2 §3.6), the seam row's three pins | O30, O53 |
| the lane contract | a lane is a segment stream a writer appends to; the request and answer rows of a judge lane; the pair contract (§29.1); the executor's statuses; fail-closed | O42, O43, O58 |
| the schema | the class table's ten lines per wire, the ingest predicates (v2 §12.1), `terminal`, `counterparty_kind`, `installments` | the schema pin |
| the battery | every oracle with its lie, as `tools/conform.py` **[v4]** runs them against a tape a second implementation produced | a second implementation is conformant when it passes the battery from the published spec alone |

**Why the standard is published** **[v4.1]**. Every firm that speaks the alphabet makes its partners' verdicts denser (§29.1) and its own floor lower by pooling (§30.1): the fourth and second dials are network effects of the standard, and the pair contract is part of it rather than an appendix to it. A conformance suite is what keeps a network of implementations one alphabet.

**Compatibility promises.** A change to a row type, reason, act kind, provenance, band count, outcome kind, admission kind or the class table moves the alphabet pin; a change to the gate moves the gate pin; a change to the instrument moves the instrument pin; a change to a template moves its pin and re-keys its licences; the segment format carries its version in the header; the JSONL projection names every field it emits and no other. **Kill F-STANDARD:** a second implementation built from the published spec alone that cannot pass the battery: the spec is incomplete and the missing clause is written before the standard is called one.

---

## §37 · Scale **[v4, BUDGET unless marked]**

| quantity | bound | source |
|---|---|---|
| the tape | 40 bytes a row; ACME's year at 500 seats is 1,607,033 rows (64 MB) **[M, D0]**; a firm at 10⁶ decision events a month with ten rows an event is 10⁷ rows a month, 5 GB a year | the segment format |
| the fold | a scan at memory speed; the chain verify at BLAKE2b's rate (of the order of 10⁷ rows a second on one core); a year's tape verifies in minutes and a cold fold runs in seconds; restarts fold the tail from the checkpoint | O14, O18 |
| the field | log-domain Sinkhorn in O(N + M) memory; O(N·k) time per sweep under the locality law with k candidate columns per promise (the contention stencil), instead of O(N·M); at 10⁴ promises and k ≈ 30 a sweep is 3×10⁵ terms, 150 sweeps a tick on one core in milliseconds; on a card, microseconds; the plan is never materialised | v2 §4.2; THE CLOSED FIRM §III.2 **[ONE]** |
| the read budget | reads ≈ arrivals + re-touched cells + the floor sample; 1,729 a period at ACME **[M, C1]**; F-READS at ~1,500 | v2 §4.2 |
| residency | warm cells ≈ (VRAM − weights − scratch)/(52.7 MB + 17 KB × tokens per cell): about 160 on a 16 GB card at 500 tokens a cell, about 2,200 on a 141 GB card **[ONE, M on FUSOR for the constants]**; the hot set is the field's top-K | THE CLOSED FIRM §III.4 |
| the archive | two million fragments scored in about two milliseconds, resident on the card **[ONE, M]**; scanned, never held | v2 §3.6 |
| the light cone | Λ = (D/c)/T; r = cT/2; 6,600 km at a 44 ms judgment, a continent at a one-second tick **[ONE, derived]**; on the header row | §30.2 |
| tenants | n₀/(N·r·V) under exchangeability with the power guard | §28.4 |
| the gym | one thread per firm; 10⁴–10⁵ firms a second per card at the search grain; 10³–10⁴ ACME-fidelity firms per card at warp grain | v2 §7.2 |
| judges | any number behind lanes; the run replays without any of them | O53 |
| processes | the kernel, the licensor, the plant or the ingester, one process per judge, the executor, the instrument, N tenants, S shards: all lanes, one merge order | §26 |

**What does not scale.** The writ is authored and does not pool. The warrant seats are the law's and do not compile out. The residual is what the machine cannot see and moves only with instrumentation. The verdict rate of a class with a person as its counterparty is the person's.

---

## §38 · Oracles and kills added **[v4]**

| oracle | asserts | the lie | step |
|---|---|---|---|
| O31 (E3, restated against the repaired rule) | the false-promotion rate under a judge at g₀, over seeds, at α | a judge above g₀ under the boundary judge's name | E3 |
| O31b | the same under a planted shared-fact flip schedule | the schedule removed under the same name | E3 |
| O31c | the same with an installment channel as the grading channel under a planted installment relation, against α plus the calibration's level | the calibration entered at its point estimates; a judge with good installments and bad verdicts | E3 |
| O45 | both ladder processes re-derive from OUTCOME rows in arrival order; the bar frozen within the term | an outcome counted after the term closed | E3 |
| O47 | the calibration curve per key is monotone; the gate reads the calibrated value; an unmeasured key reads rung 0 | a non-monotone judge admitted | E3 |
| O42 | the fold over lanes equals the fold over the flattened tape; every SEAL re-derives | a late row folded into a sealed period | F0 |
| O43 | a silent licensor holds everything with `no-stratum`; a STOP row returns the next period before it reads | a cell that acts without a STRATUM row | F0 |
| O44 | every BREACH re-derives from rows plus the pinned instrument; a planted shadow lie is caught within its period; a miss forfeits the stake | an instrument told to skip `prov`-instrument rows | F0 |
| O53 | a run replayed with every judge lane absent reproduces every non-judge row bit for bit | a verdict that consulted a judge during replay | F0 |
| O55 | every RESERVE has an EFFECT or an UNDO; unknowns reconciled or written off within the term; exposure counts unknowns | an unknown receipt treated as executed | F0 |
| O58 | the two-process sim is byte-identical to the in-process run | a lane that reorders proposals | F0 |
| O39 | predicted D_c per class within tolerance of measured days to rung 1, or `never` with the reason | the formula fed the planted p; or L doubled without D_c recomputed | F1 |
| O40 | a bootstrap band acts only on canary cells of reversible, non-warrant classes, never past the bootstrap cap, with its admission kind on the LICENSE row | the rule applied to an irreversible class | F1 |
| O41 | with every judge lane absent the floor runs the wire: terminals at due, nothing above rung 1, the bar folded from the floor's outcomes | a terminal before due, or a floor act above rung 1 | F1 |
| O48 | INSTALLMENT rows always feed the field and the demotion process; they enter the evidence process only as a class's declared channel under a calibration frozen at its bounds, one factor per cell, the cell's final OUTCOME excluded from that term's evidence process; acceptance-graded classes receive OUTCOME at commit | an installment entering the evidence process without a frozen calibration, or with the calibration at its point estimates | F1 |
| O49 | on a paired class every OUTCOME joins the counterparty's EFFECT by the key at its commit tick; none is written by the firm's own port | an OUTCOME on a paired class written by the firm's port | F1 |
| O56 | the anomaly list and the account re-derive from EXPECT, OUTCOME and the ledger's rows | an anomaly without an EXPECT row | F1 |
| O52 | a do() intervention changes only the world rows downstream of its cell | an intervention that reseeds the world | H |
| O24 | two tenants interleaved produce the rows of two separate runs | a tenant's row folded under the other's id | H2 |
| O50 | a pooled promotion satisfies the exchangeability test, the power guard, and each tenant's own processes | pooling by matching hashes alone | H2 |
| O51 | a grafted instance's licence table is rung 0 everywhere; a spore carrying state is refused | a spore carrying the licence table | H2 |
| O54 | a routine reproduces the judge on its history and on every retained cell since; one disagreement revokes it | a routine kept after a disagreement | H2 |
| O57 | the JSONL projection round-trips to the chain head; a second implementation's tape folds to the same ledger and passes the battery | a JSONL row with an unnamed field accepted | I |

Kills, continuing v3's numbering, which ended at 19:

20. **F-SELECT.** A world chosen by the selection rule whose head classes fail to license within three times their predicted D_c: the floor is not identifiability, or the rule is mis-specified.
21. **F-BOOT.** Realized loss under the bootstrap above its cap, or a false-promotion rate above α on bootstrap bands: the allowance is withdrawn.
22. **F-INSTALL.** An installment channel whose frozen calibration leaves the evidence process without power, or whose false-promotion rate under O31c exceeds its level, leaves the evidence process and stays in the field and the demotion process.
23. **F-POOL-POWER.** Pooled tenants at small windows with a false-promotion rate above α: the guard tightens or pooling is withdrawn.
24. **F-LANE.** The two-process sim not byte-identical, or slower by more than three times: the lane contract is wrong.
25. **F-GERM.** A grafted instance that re-earns no faster than a fresh one: the spore carries nothing.
26. **F-DENSE.** A dense head that beats the naive rate on next-world-row loss and moves neither licensed mass nor cost: learning dense buys forecasting only, said in advance.
27. **F-STANDARD.** A second implementation from the published spec alone that cannot pass the battery: the spec is incomplete.

Also run in v4's steps, from the registers of v2 §15, v3 §22 and the ONE corpus: F-STUB (E3), F-CALIB (E3), F-READS (E1), F-RESIDENT (H), F-TENANT (H2), F-COMPILE (H2), F-TWINS-OF-HISTORY (H), F-HOSTILE (H), F-EMBED (E2), F-SIM2REAL (I), F-VECTOR-KEY (every step).

---

## §39 · The build order, amended **[v4]**

The order of v3 §23 stands through E2. v4 inserts E3 before any step that adds a way to act, then F0 (which moves no number) before F1 (which moves many), and H2 after H.

| step | v4 content | acceptance | numbers |
|---|---|---|---|
| **E0** landed | as v3 §23 | the step-E receipt | moved, named |
| **E1** | as v3 §23: the counsel memo (O23), VOC, the retained stratum (O20), novelty on the frame with the conformal fix (O22), O-TEMPLATE, O-GEOM′, the coupling print, the latency-and-value sweep; the frame embedded behind an embedder lane | numbers move and are named | move |
| **E2** | as v3 §23: the archive plane, `SPAN`, `WANT`, `RF_DISCOVERED`, the standing-question index, the reranker behind its adapter, the synthetic corpus per class; O32–O35 with the planted embedder (§32.4); F-EMBED | the controller blind to the plant; recall, missed cells, stale proposals, incorrect reuse, cost per verified completion printed | move |
| **E3 [v4]** | **E3a landed 2026-09-15**: `--o31` and the boundary judge; the ladder as built measured at 13.7 percent false promotions against a stated 5 (the "before"). **E3b landed 2026-09-15**: the ladder v2 (§28.2) as the only ladder; the bar frozen; one process per term; mirrored demotion; term and lapse above the admitted floor; each outcome graded against its act's term (F24); the retained stratum at every rung (O20, F14); REGIME (O59); O31 restated as the standing share with the Bernoulli boundary check, O31b, O45; the LICENSE row's cause on its via byte. **E3c open**: the calibration harness (§28.5) with the gate reading the curve, O47; F-STUB and F-CALIB run. O31c with the installment channel of §29.3 lands with the channel in F1b. The bootstrap admission kind and the floor's terminal are defined in the schema but exercised only in F1 | the standing share at its level over seeds, under independence and under the shared fact; licensed mass smooth in competence; every delta from E0 named (the ladder moves numbers) | move |
| **F0 [v4]** | lanes and SEAL (§26); `acme-licensor` as a process with the salt, the stake, STOP, REGIME (§27); fail closed; RESERVE and `--executor-loss` (§34); the instrument lane and the production battery (§27.3) with `tools/instrument_hash.py`; `--replay-from` (§32.5); the two-process sim (§26.5); O42, O43, O44, O53, O55, O58; the alphabet and gate pins move and are named | **every reading identical to E3 under `live`** in-process and across processes; a licensor kill survived (every cell holds); a run replayed with no model; O1b across processes | identical |
| **F1 [v4]** | the fifth world (§25): `WorldParams` from selection parameters, `--select`, `--greenfield`, the bootstrap rule, the floor as the null arm, the jury lane as veto, `--absent N`; `--pair` and INSTALLMENT (§29); `--wire-order` and the dial on the binding-reason print (§25.7); EXPECT rows, `--anomalies`, `--backtest`, `--account` (§35); O39 with its curve, O40, O41, O48, O49, O56; F-SELECT and F-BOOT run; the three worked businesses and ACME's own world as world files | ACME's own world reproduces E3 byte for byte under `WorldParams`; business A's head licenses within predicted D_c and business C prints c_c ≤ 0; a paired class's latency collapses to the tick; the wire order's licensed-mass curve against `--select`'s projection; the week with nobody home at planted grade | **move; the first measurement of the act loop on a world that answers fast** |
| **G0, G** | as v3 §23: the schema as data; the census on one real lane, no judge, no effect; the lane's archive folded into `SPAN` rows | the compiler meets a world | none |
| **H** | as v3 §23: the gym and the fitted world; the multiverse as the judge's data generator with `--export` and `--do` (§32.3); hostile worlds; F-TWINS-OF-HISTORY; F-HOSTILE; F-RESIDENT (the tick sweep); commit grain against the full stream; O26, O52 | sim-to-real measured, not assumed; hostile worlds read as hostile | none until O26 |
| **H2 [v4]** | tenancy (§30.1; O24, O50; F-TENANT); Λ on the header and shards at the seam (§30.2); lineage (§31; O51; F-GERM); compile-out and the intelligence budget (§33; O54; F-COMPILE); the anomaly schedule in the writ | two tenants license a band neither licenses alone; a grafted instance at rung 0 re-earning; a routine compiled, checked and revoked on the stratum; the budget curve printed | move, named |
| **I** | as v3 §23 with the fourth review's items, the `TapeJudge` behind a judge lane (O36, F-SIM2REAL), the `RECEIPT` statuses; plus the standard's conformance suite (§36; O57; F-STANDARD); `intellect` at `off` on the first real lane with `acme-licensor` beside it; replay with a real judge; the first canary at the floor under a bootstrap rule where there is no history | the first exterior verdict; **no lane `live` until O31 and O31b have passed** | the number that matters |

Each sub-step: build, the battery, every lie arm, the tool oracles, the three modes diffed against the previous stage, a receipt section, the docs (the receipt, the README row and table by `tools/oracle_table.py`, the spec's landed notes and its mirror, `CLAUDE.md` = `AGENTS.md`, the memory), a commit, a push. One sub-step at a time, each on the operator's word. G0 and G may start any time after B, as v2 §14 already allows: the census needs no judge, no ladder and no lane, and nothing in v4 gates first contact with a world on E3 or F0.

### 39.1 The incumbent's path, mapped **[v4.1]**

NINETY PERCENT gives the forward-deployed engineer six deliverables and the enterprise five phases. Each is a v4 mechanism at a switch state, and none needs a mechanism v4 lacks.

| deliverable (NINETY PERCENT §XII) | phase | v4 mechanism | step |
|---|---|---|---|
| the census: change capture on every system of record, the ten lines per class, the history replayed, the flattening dividend and D_c per class from the firm's own history | 0 | the schema as data; the CDC ingester as lane 0; the fold; `--select` from the census's replay and the wire order (§25.7) | G0, G |
| the plane with no model: every open promise visible; the meetings and the coordinator roles retired; coordination minutes measured before and after | 1 | INTELLECT at `off` with the observer lane and `--account` (§35) | G |
| the templates: each class's context compiled once from the fossil by the frontier, tightened by ablation in the gym; the cognitive index | 2 | the compile step (v2 §4.6) behind a frontier lane; E2's archive plane; the template pins | E2, H |
| the writ and the rules: δ, the caps, the floors, the retained fraction, the exposure cap; one licensing rule per class, signed once; the bars frozen from the shadow record | 2 | the writ file; the bootstrap rule as an admission kind (§25.3); the bars and the calibration record (§28) | E3, I |
| the switch, one wire at a time: `off`, then `shadow`, then `live` on the draft loop, then the act loop with canaries at the floor on a salt the machine cannot see, the retained stratum forever, the stable classes compiled | 3 | the switch file (D2); `--wire-order` (§25.7); the ladder v2; the licensor as a process (§27); routines (§33); the first outcome row | I |
| the second wire and the second client: a configuration change; pooled outcomes under one salt; counterparties moved onto the schema; installments calibrated | 4, 5 | tenant lanes and the schema licensor (§30.1); the pair contract and the grading channel, negotiated wire by wire (§29) | H2 |

The forward-deployed engineer's instrument is this binary's flags, and the metric is supervision minutes and κ, never adoption or hours saved.

### 39.2 The order as confirmed · 2026-09-15

On the operator's confirmation of the session's recommendation, this document is the standing design and the order from E0 is: a QC commit (the README and the handoff brought to E0; this document staged); **E3** first, with the retained stratum (O20, F14) pulled into it from E1 because the frozen bar needs incumbent decisions in every band at every rung, opened by an O31 reading against the ladder as built so the receipt has a before and an after; then **F0**; then **F1a** (`--select`, `WorldParams`, the greenfield world with the bootstrap rule and the floor; O39, O40, O41; F-SELECT run) with `--pair`, installments and the account following as **F1b**; then E1′ (the counsel memo, VOC, novelty on the frame, O-TEMPLATE, O-GEOM′) and E2; then G0 and G, H, H2 and I as §39 lists them. G0 starts in parallel on a real lane at `off` as soon as one is named. No further version of this document until something in it has a receipt.

**The ladder of closure tests** (THE CLOSED FIRM §XVII), with the status column as of this version:

| rung | test | grader | status |
|---|---|---|---|
| 0 | one judgment replayable | O1b, O30 | landed, planted grade |
| 1 | a full loop at one seat | the sim's period; O14, O18, O19 | landed, planted grade; unrun on a real lane |
| 2 | the interior loop | E1, E2 | unrun |
| 3 | a week with nobody home | `--absent 7` (F1); O41, O43, O44 | unrun; runnable at planted grade after F1 |
| 4 | a wire with the person outside the loop | I | unrun |
| 5 | the constitution replaces the signer | F0 + I; `acme-licensor` on separate hardware with the stake | unrun; the process exists after F0 |
| 6 | federation | H2; O42 across shards, O50 | unrun |

---

### 39.3 Landed notes · E3a and E3b, 2026-09-15 **[v4.1]**

**E3a** built the instrument §38's O31 needs and read the rule as built with it: a boundary judge the plant holds on a stated null (the incumbent's rate, or a delta worse), every band admitted to the wager as a test. The rule promoted a judge a delta worse than the incumbent on **13.7 percent of bands at 500 seats (Wilson 9.5 to 19.3)** against a stated 5. F23 is a reading.

**E3b** replaced the rule with §28.2's ladder v2 and found, in the plant, three things the paper's statement does not survive; each is now in §28.2 and in the code:

- **F24 · the act's term grades the outcome.** The bar moves between terms because the paired arm's rate is not stationary under the machine's presence; an outcome arrives one latency later; graded against the arrival term's bar a null judge promotes whenever the bar falls in between (a judge three points inside every term's g₀ promoted on half the bands). The ladder keeps its frozen bars term by term and grades each outcome against the bar of its act's term. The bound stands; the plant's judge then promoted nothing.
- **The lapse floor.** Re-demanding the admitted rung every term at 1/α₁ lapses every band of modest volume at the first boundary. Lapse applies to earned rungs, never below the admitted floor.
- **Demotion at the point bar.** Tested against the upper bound, a machine as good as the incumbent is demoted; the mirrored process faces the point estimate.

**The reading that moves the programme's number.** At 500 seats licensed decision mass falls from **63.5 to 13.1 percent**, N from 193 to 437, bands past rung 0 from 29 to 11. The old number rested on the broken rule and on a paired baseline measured on the cells the machine left to people (the leftovers): `quote.price` band 2 read the incumbent at 0.191 under E0 and reads 0.849 on the shadow record and the retained stratum under E3b, and the machine at 0.709 is correctly demoted. F14, F21 and F23 were one mechanism. The stub sweep (F-STUB) across the plant judge's competence 0.40 to 0.96 moves licensed mass by 2.6 points: **at this schema the judge is not the lever; the world's verdict rate is**, which is the fifth world's question and the reason the first real judge belongs on the greenfield world.

**Kills added at E3b:**

- **F-JUDGE-MATTERS.** On the greenfield world, a real judge behind a lane must move licensed mass by more than the stub sweep's spread at matched calibration, or the model is not the lever at that schema either and the programme's money stays on the four dials.
- **F-BODY** (for the harness as INTELLECT's body, §26): if the body costs more supervision minutes at the console than it removes on the operator's own desk, the body is scaffolding.

**E3c** built §28.5 as stated, with one reading worth the sentence: the curve is measured on the cells whose executed choice was the machine's (the history's agreement cases, then every live outcome via 1, 2 and 4 and via 0 where the person chose what the proposal said), frozen per term and written as CALIB rows the ledger folds, so the gate reads the same curve a cold fold reads and O30 re-derives every effect through it; an unmeasured or non-monotone key drafts with the reason `uncalibrated`; the monotone test is three standard errors and five points, because two standard errors over twenty-eight pairs per class flags an honest judge by chance. O47's lie is a judge whose choice is flipped where its confidence is highest on one class, admitted there by a resident that ignores the flag.

**E4, the account**, landed 2026-09-15 the same day: the instrument panel as folds over rows (`panel.h`), moving no reading: the five metrics, CPI for the firm by bucket, the roofline per class with the binding roof, the fusion ledger by reader class, the jury meter, the validity record; the frontier's read as a COUNSEL row; O48 (the panel folds from the tape) with its lie. Three readings at 500 seats: the frontier is rented 5.07 times per unattended act (22,576 rentals on 5,377 distinct cells; the counsel memo of E1′ buys about 17,000 a year); the legal fusion dividend by minutes is 54.0 percent (static glue 33.7, meetings 17.3, transfer 3.0) against the 0.61 to 0.77 by writes; the jury meter says a dissimilar-but-dumb juror (the rules coin, 49.9 percent disagreement, enrichment 1.02×) is noise while a competent sibling (6 to 23 percent disagreement, enrichment 4 to 69×) disagrees where the errors are, so §29's jury must be competent and of another family, and the meter is how it is admitted. The memo's hit rate is 53.7 percent of judgments (an earlier prose figure of 84.5 was the wrong ratio).

### 39.4 Landed notes · F0a, 2026-09-15 **[v4.1]**

**F0a built §26's lanes as a projection before splitting them across processes**, which is the order that finds the defects cheaply: `lanes.h` holds the six-lane roster and, new here, the **nine-phase order of a period**, both pure functions of the row, so a tape written as one stream splits into lanes exactly and merges back. `R_SEAL` closes each period over every lane, carrying the merged head; `split_lanes` reproduces those heads from the rows alone; `merge_lanes` folds to the same ledger as the flat tape; O42 measures all three and its lie is a period's arrivals folded late.

**The correction to §26.2, found by the sim before a line of the two-process build.** §26.2 orders the merge by lane: every lane l ≥ 1 in lane order, then lane 0. That cannot hold, because **two writers each write twice in a period**: the world port opens it (TICK, ARRIVE) and closes it (OUTCOME, SEAL), and the licensor draws the strata before the machine reads and moves the ladder after the outcomes land. Fold lane 0 last and an ARRIVE lands after the EFFECT on the cell it opened. The merge key is therefore **(day, phase, lane, sequence)** with the nine phases published as part of the alphabet: open, arrive, draw, read, gate, human, settle, ladder, close. A writer may appear in several phases; a phase may hold several lanes; within a phase the lanes fold in lane order. §26.2 is amended and the oracle now holds it.

**Two laws the step restated rather than invented.** A **NOTE is a row of no period** (D1's law, which is what made a resumed tape equal an unkilled one), so it is a row of no seal: it joins its lane's rows and never its lane's chain, or a restore marker moves every later merged head and O18 dies of its own bookkeeping. And the **seal's merged head is a chain over every byte**, including the operator's wall reading on the TICK row, so under a jittered clock the seals differ and no verdict does; O25 says so in its own line rather than pretending otherwise.

**The reading.** At 500 seats the year's 413,140 rows divide as world 88,045 · licensor 51,885 · judge 53,849 · kernel 53,683 · human 165,676 · executor 0, over 70 seals, every one re-deriving from a cold split, with no row of a sealed period written after its seal. 160,060 rows sit at a different index under the merge than in the flat stream, which the fold does not read: that number is the honest size of the reordering a multi-writer tape permits, and the fold's invariance across it is what makes a lane a protocol rather than a copy. The executor lane is empty until F0c mints receipts.

**Notes for F0 from the components brainstorm of 2026-09-15** (`C:\55555\BRAINSTORM_ACME-REAL-COMPONENTS_FUSORD-EMIT-CUDA-DSH_2026-09-15_FABLE5-1.md`): adopt fusord's lane contract v0.1 as the wire for every text lane, with a `row` grain for the typed Rec; every real component (the resident kernel, the emit model, the CUDA gym, the harness) mounts on a lane or around the kernel as a body, none is linked into `acme`; the licensor is never a plugin.

## §40 · The ledger of this version **[v4]**

**Measured, unchanged by this document:** everything in §24.1; every reading in Part II. Nothing in v4 moves a number in any receipt.

**What v4 makes runnable at planted grade, and when:** the false-promotion rate, under independence, under the shared fact and with an installment channel (E3); the selection rule and the act loop on a world that answers fast (F1); the collapse of a paired class's verdict latency to the tick (F1); the week with nobody home (F1); a run replayed with no model (F0); the sim as processes over lanes (F0); a grafted instance against a fresh one (H2); two tenants licensing a band neither licenses alone (H2); a routine compiled, checked and revoked (H2); the counterfactual dataset with do() (H).

**What v4 cannot make runnable:** the first outcome row on a real lane (step I; there is no synthetic substitute for a wire with a future) and the twin race between a resident center and a tick-driven one (it needs a model behind a lane, and the corpus is right that it is decisive for the center). Spore against tape as a session experiment stays the corpus's.

**Unrun, all of it:** every oracle numbered above 38, every kill numbered above 19, every reading §25.6 and §39 promise. The status column is the honest one.

**What would show v4 wrong at its root.** If F0 cannot be made byte-identical across processes, the seam is not a protocol and the kernel is not the thin waist it claims to be. If the greenfield world at business A's parameters does not license its head within three times its predicted D_c, the corpus's lever is a story and ACME's act loop was never waiting for a faster world. If the ladder v2's false-promotion rate under the shared fact is not at α, the anytime guarantee does not survive the world E0 built, and no lane goes live until it does.

---

## §41 · The line

*One machine, five worlds, one tape, many lanes. The closed firm is a world behind the port, generated from the numbers that choose it, with a floor for its null and a rule the owner signs once for its first act. Every writer is a lane and the fold is a merge, so the licensor is a process the machine cannot reach, a tenant is a lane, a shard is a lane, and the other firm's commit is this firm's verdict. The ladder is repaired before anything new may act, and the gate reads a curve and never a model. The verdict rate has four dials, and each is code the owner can read. A run replays with no model present, which is what it means for the tape to be the company. The instrument runs on the live lane with its lies planted and a stake behind it. The alphabet, the tape and the battery are a standard, because the floor clears by pooled volume and pooling needs a language. The top rung is a set of oracles with their lies, and every one of them is unrun. The tape is the proof.*

---
*v4.1 · 2026-09-15 · Claude Fable 5.1. Supersedes nothing; a superset of v3, which is a superset of v2. Revised the same day after NINETY PERCENT (§24.5). Part II follows, verbatim. Nothing in Part I is a measurement.*

---
---

# Part II · ARCHITECTURE v3 as landed, verbatim

*Everything below this line is `docs/ARCHITECTURE_v3.md` at commit `ea9ea74` on branch `v2`, included unedited so that this file is a superset. Its Part I is §18–§23 and its Part II is v2 verbatim (§0–§17, §14a, §14b, kills 1–16); v3's kills 17–19 are the kills v4's 20–27 continue; v4's own sections are §24–§41 above.*
