...
- **Term and lapse.** At the term's end a rung is kept only if the term's process reached its threshold; otherwise the band drops one rung per term of non-attainment. A LICENSE row records every change with its admission kind.
...
**O31 · the false-promotion rate.** Over S seeds on a synthetic firm whose judge sits exactly at g₀ in every band, the fraction of class-bands that ever reach rung 1 within a term, with its binomial interval, against α. The lie: a judge above g₀ under the boundary judge's name. **O31b · under correlation** **[v4]**: the same, with a planted shared-fact flip schedule (`--shared --flip-rate`) that correlates outcomes within epochs; the rate must hold at α. The lie: the flip schedule removed under the same name. **O45 · the ladder is a fold** **[v4]**: both processes re-derived from OUTCOME rows in arrival order equal the licensor's; the bar's recorded value is unchanged within the term. The lie: an outcome counted after the term closed.
...
| **E3 [v4]** | the ladder v2 (§28.2) as the only ladder; the bar frozen; one process per term; mirrored demotion; term and lapse; REGIME; the calibration harness (§28.5) with the gate reading the curve; O31 restated, O31b, O31c with the installment channel of §29.3 under a planted installment relation, O45, O47; F-STUB, F-CALIB and F-INSTALL run; the bootstrap admission kind and the floor's terminal defined in the schema but exercised only in F1 | the false-promotion rate at its level over seeds, under independence, under the shared fact and with an installment channel; licensed mass smooth in competence; every delta from E2 named (the ladder moves numbers: F21's six bands re-earn) | move |
