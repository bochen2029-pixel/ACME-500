...
About 6,900 lines on branch `v2` (v1 is tag `v1-step-A`). Seven modes. Thirty-three oracles in the binary and four on the source tree, each carrying a lie that it must be seen to catch. Every number it prints is a reading, and the readings that turned out wrong are printed as retractions in the receipts.
...
| O1b | same seed produces a byte-identical MACHINE arm: tape, chain, licence table | PASS, 409259 rows; rec same, chain same, licence table same |
| O14 | the ledger folds from the tape: cold fold == the live ledger every consumer reads | PASS, 58367 cells folded from 409259 rows; 0 field diffs; 0 minute diffs; alphabet pin matches |
| O29 | every row carries what the v2 table says it carries | PASS, 409259 rows scanned, 0 shape faults |
...
| O18 | a kill survives: the durable tape reopens, the checkpoint restores, the run continues bit for bit | PASS, cut at row 361156 + 17 bytes with checkpoint 59 in flight; restored at day 49, 59630 rows dropped; continued run: 409260 rows, chain verified, 0 fold diffs, rows equal the unkilled run's |
...
| O25 | the same rows at different wall spacing give identical verdicts: the machine reads the day, never the clock | PASS, 409259 rows against 409259; 70 TICK rows carry a different wall value; every other byte identical |
...
| O31 | the ladder v2 leaves a judge inside the null with unearned authority no more of the time than its level (at the exact boundary in a Bernoulli world 183 of 20000 band-terms crossed the first earned rung's threshold, rate 0.0092 against alpha/4 = 0.0125, bound 0.0140; the plant: 3 seeded firms, 114 bands run, the judge on each term's frozen g0 less three points: over the 8965 settled wager cells the process graded, expected good rate 0.3778 from its coins, observed 0.3869, the g0 of their acts' terms 0.5046, so INSIDE the null; 0 ever promoted; 0 of 432 band-terms standing above the test rung, share 0.0000 against alpha 0.05, bound 0.0706; against each band's lowest g0 across terms (mean spread 0.272): 39 bands below it with 0 promoted, 43 above it with 0 promoted) | PASS |
| O31b | the standing share holds under a shared fact that correlates outcomes within its epochs (the shared fact moved; 3 seeded firms, 114 bands run, the judge on each term's frozen g0 less three points: over the 9119 settled wager cells the process graded, expected good rate 0.3667 from its coins, observed 0.3689, the g0 of their acts' terms 0.4832, so INSIDE the null; 0 ever promoted; 0 of 432 band-terms standing above the test rung, share 0.0000 against alpha 0.05, bound 0.0706; against each band's lowest g0 across terms (mean spread 0.275): 40 bands below it with 0 promoted, 43 above it with 0 promoted) | PASS |
...
| O30a | the gate is pinned: include/acme/gate.h digests to its pin 471196c2f73ff4f3 | PASS |

*Regenerated from `receipts/step-e-2026-09-15/e3b/selftest.txt`, `o28_o25a.txt`, `o17.txt` and `o30a.txt` by `tools/oracle_table.py`; the readings are the run's own lines.*
...
include/acme/gate.h       THE GATE, a hashed library: the verdicts, the fifteen reasons, the alphabet pin, the switch; its pin in gate_hash.inc (O30a)
