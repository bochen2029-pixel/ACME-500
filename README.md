...
About 7,200 lines on branch `v2` (v1 is tag `v1-step-A`). Seven modes. Thirty-four oracles in the binary and four on the source tree, each carrying a lie that it must be seen to catch. Every number it prints is a reading, and the readings that turned out wrong are printed as retractions in the receipts.
...
| O1b | same seed produces a byte-identical MACHINE arm: tape, chain, licence table | PASS, 409643 rows; rec same, chain same, licence table same |
| O14 | the ledger folds from the tape: cold fold == the live ledger every consumer reads | PASS, 58367 cells folded from 409643 rows; 0 field diffs; 0 minute diffs; alphabet pin matches |
| O29 | every row carries what the v2 table says it carries | PASS, 409643 rows scanned, 0 shape faults |
...
| O18 | a kill survives: the durable tape reopens, the checkpoint restores, the run continues bit for bit | PASS, cut at row 361348 + 17 bytes with checkpoint 59 in flight; restored at day 49, 59630 rows dropped; continued run: 409644 rows, chain verified, 0 fold diffs, rows equal the unkilled run's |
...
| O25 | the same rows at different wall spacing give identical verdicts: the machine reads the day, never the clock | PASS, 409643 rows against 409643; 70 TICK rows carry a different wall value; every other byte identical |
...
*Regenerated from `receipts/step-e-2026-09-15/e3c/selftest.txt`, `o28_o25a.txt`, `o17.txt` and `o30a.txt` by `tools/oracle_table.py`; the readings are the run's own lines.*
