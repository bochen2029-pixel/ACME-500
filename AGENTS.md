...
- Reproduce the battery before editing: 17 of 17 in `--selftest`, every `--lie N` run (0 to 14; 14 is O1b) exiting 0 with the lied-to oracle's own line reading PASS, and `python tools/dead_symbols.py .` PASS on both arms (O28). The REV 2 spec in `docs/SPEC-AMENDMENTS_REV2_ONE-PORT-FOUR-WORLDS.md` is the standing kickoff; its §0 is the regression baseline, and `receipts/RECEIPT_ACME500_STEP-A_2026-09-14.md` is the last step landed.
